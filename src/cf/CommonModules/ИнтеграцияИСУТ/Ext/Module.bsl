...
Процедура ОпределитьВидПродукцииТекстаЗапроса(ТекстЗапроса,
	...
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
...
Процедура ОпределитьШаблоныКодовМаркировкиСУЗТекстаЗапроса(ТекстЗапроса,
	...
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.МолочнаяПродукция)
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.Шины)
...
Процедура ЗаполнитьДанныеПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоШтрихкодВводаОстатков(СтрокаДанных.ШтрихкодEAN, Истина) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЭтоВесовойШтрихкод Тогда
			ДополнительныеПредставленияШтрихкодов.Вставить(СтрокаДанных.ШтрихкодEAN, ТекШтрихкод.Штрихкод);
			ШтрихкодыEAN.Добавить(ТекШтрихкод.Штрихкод);
		Иначе //в информационной базе может быть записан GTIN
			ДополнительныеПредставленияШтрихкодов.Вставить(СтрокаДанных.ШтрихкодEAN, ШтрихкодированиеИСКлиентСервер.GTINПоШтрихкодуEAN(СтрокаДанных.ШтрихкодEAN));
			ШтрихкодыEAN.Добавить(ШтрихкодированиеИСКлиентСервер.GTINПоШтрихкодуEAN(СтрокаДанных.ШтрихкодEAN));
		КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		ШтрихкодПоиска = ДополнительныеПредставленияШтрихкодов.Получить(СтрокаДанных.ШтрихкодEAN);
...
Функция ДанныеПеревозки(Основание, ТранспортнаяНакладная) Экспорт
	...
	Иначе
		...
		|	ЗаданиеНаПеревозку.КурьерЭкспедитор,
...
Процедура ЗаполнитьПоПараметрамСоздания(Объект, ПараметрыСоздания) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.ПроизводительИмпортерДляДекларацийАлко) 
		...
		Если ПараметрыСоздания.Свойство("ИмпортерЕГАИС")
			...
			Объект.ПроизводительИмпортерДляДекларацийАлко = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСоздания.ИмпортерЕГАИС,
																										"Контрагент");
		Иначе
			Объект.ИмпортнаяАлкогольнаяПродукция = Ложь;
			Объект.ПроизводительИмпортерДляДекларацийАлко = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСоздания.ПроизводительЕГАИС,
																										"Контрагент");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.ВидНоменклатуры)
		...
		|	ИЛИ ВидыНоменклатуры.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)";
...
#Область Реквизиты
	...
	ДобавляемыеРеквизиты.Добавить(Колонка);
	Колонка = Новый РеквизитФормы("СтатусПроверкиГосИС", Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(1)),"Товары");
	ДобавляемыеРеквизиты.Добавить(Колонка);
...
#Область Команды
	...
	Кнопка.Картинка              = БиблиотекаКартинок.МаркируемаяАлкогольнаяПродукцияШапка;
