...
Функция СведенияОСопоставленииНоменклатурыБизнесСеть(Номенклатура) Экспорт
	...
	|	СоответствиеНоменклатурыБизнесСеть.ОбъектСопоставления КАК Объект
	|ИЗ
	|	РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеНоменклатурыБизнесСеть
	|ГДЕ
	|	СоответствиеНоменклатурыБизнесСеть.ОбъектСопоставления = &Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	""Родитель"",
	|	СоответствиеНоменклатурыБизнесСеть.ОбъектСопоставления
	|ИЗ
	|	РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеНоменклатурыБизнесСеть
	|ГДЕ
	|	СоответствиеНоменклатурыБизнесСеть.ОбъектСопоставления = &Родитель
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеНоменклатурыБизнесСеть
	|ГДЕ
	|	СоответствиеНоменклатурыБизнесСеть.ОбъектСопоставления = &ВидНоменклатуры";
...
Функция ПолучитьПубликуемыеСвойства(ТаблицаСвойств, Номенклатура, Характеристика,
	...
	Запрос.УстановитьПараметр("ТолькоСопоставленныеСвойства" , ТолькоСопоставленныеСвойства);
	
	Если ВариантСопоставления = "РаботаСНоменклатурой" Тогда
		Запрос.Текст = ТекстЗапросаСвойстваРаботаСНоменклатурой();
	ИначеЕсли ВариантСопоставления = "БизнесСеть" Тогда
		Запрос.Текст = ТекстЗапросаСвойстваБизнесСеть();
	КонецЕсли; 
	...
	Для каждого СтрокаКатегории Из ТаблицаКатегорий Цикл
		Для каждого СтрокаДополнительногоРеквизита Из СтрокаКатегории.ДополнительныеРеквизиты Цикл
			...
			Попытка
				...
				ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СоответствиеРеквизитовБизнесСеть");
...
//    * МассивСсылок - Массив из ОпределяемыйТип.НоменклатураРаботаСНоменклатурой ИЛИ 
		...
		Пока ВыборкаСостав.Следующий() Цикл
			...
			Для каждого СтрокаОтвета Из СтрокиОтвета Цикл
				...
				Если НЕ ПустаяСтрока(СтрокаОтвета.description) Тогда
					...
					КонецЕсли;
					ОписаниеОшибки = ОписаниеОшибки + СокрЛП(СтрокаОтвета.description);
				КонецЕсли;
				Если Состояние = 1 И ЗначениеЗаполнено(СтрокаОтвета.gtin) Тогда
					// gtin-ы записываем только для состояния = 1 "Принята"
...
Процедура ЗаполнитьКэшКатегорийДаннымиСервиса(ТаблицаКатегорий, КэшКатегорий, ОбновитьДанные = Ложь)
	...
	Если Не ЗначениеЗаполнено(КэшКатегорий) И ЗначениеЗаполнено(ТаблицаКатегорий) Тогда
		
		СтруктураКорневогоРаздела = Новый Структура;
		СтруктураКорневогоРаздела.Вставить("Идентификатор",                 "");
		СтруктураКорневогоРаздела.Вставить("ИдентификаторРодителя",         "");
		СтруктураКорневогоРаздела.Вставить("Наименование",                  НСтр("ru='Все категории'"));
...
Процедура ЗаписатьЗначениеСопоставления(Знач ОбъектыСопоставления, ВыбраннаяКатегория, Отказ) Экспорт
	...
	ОчиститьСопоставлениеКатегорий(ОбъектыСопоставления, Отказ);
	...
	Попытка
		...
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("ИдентификаторКатегории", ВыбраннаяКатегория.Идентификатор);
		Блокировка.Заблокировать();
		
		НаборЗаписей = РегистрыСведений.СоответствиеНоменклатурыБизнесСеть.СоздатьНаборЗаписей();
		Для Каждого ЗначениеМассива Из ОбъектыСопоставления Цикл
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.ОбъектСопоставления    = ЗначениеМассива;
			НоваяЗапись.ИдентификаторКатегории = ВыбраннаяКатегория.Идентификатор;
			НоваяЗапись.ПредставлениеКатегории = ВыбраннаяКатегория.Наименование;
		КонецЦикла;
...
//
// Параметры:
//  ОбъектыСопоставления - Массив из ОпределяемыйТип.ОбъектыСопоставленияБизнесСеть- ссылки на сопоставленные 
...
Процедура ОчиститьСопоставлениеКатегорий(Знач ОбъектыСопоставления, Отказ) Экспорт
	...
	Попытка
		...
		|	И СоответствиеРеквизитовБизнесСеть.ИдентификаторРеквизитаКатегории <> """"
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ТекстЗапросаСвойстваБизнесСеть()
	...
	|	ВЫБОР
	|		КОГДА НЕ СоответствиеНоменклатуры.ОбъектСопоставления ЕСТЬ NULL
	|			ТОГДА СоответствиеНоменклатуры.ОбъектСопоставления
	|		КОГДА НЕ СоответствиеРодителя.ОбъектСопоставления ЕСТЬ NULL
	|			ТОГДА СоответствиеРодителя.ОбъектСопоставления
	|		КОГДА НЕ СоответствиеВидаНоменклатуры.ОбъектСопоставления ЕСТЬ NULL
	|			ТОГДА СоответствиеВидаНоменклатуры.ОбъектСопоставления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ОбъектСопоставления
	|ПОМЕСТИТЬ ОбъектыСопоставления
	|ИЗ
	|	ТаблицаНоменклатуры КАК ТаблицаНоменклатуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеНоменклатуры
	|		ПО ТаблицаНоменклатуры.Номенклатура = СоответствиеНоменклатуры.ОбъектСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеРодителя
	|		ПО ТаблицаНоменклатуры.Родитель = СоответствиеРодителя.ОбъектСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеВидаНоменклатуры
	|		ПО ТаблицаНоменклатуры.ВидНоменклатуры = СоответствиеВидаНоменклатуры.ОбъектСопоставления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СоответствиеРеквизитовБизнесСеть.РеквизитОбъекта КАК РеквизитОбъекта,
	|	СоответствиеРеквизитовБизнесСеть.ИдентификаторРеквизитаКатегории КАК ИдентификаторРеквизитаКатегории,
	|	СоответствиеРеквизитовБизнесСеть.ПредставлениеРеквизитаКатегории КАК ПредставлениеРеквизитаКатегории
	|ПОМЕСТИТЬ ВсеРеквизиты
	|ИЗ
	|	РегистрСведений.СоответствиеРеквизитовБизнесСеть КАК СоответствиеРеквизитовБизнесСеть
	|ГДЕ
	|	СоответствиеРеквизитовБизнесСеть.ОбъектСопоставления В
	|			(ВЫБРАТЬ
	|				ОбъектыСопоставления.ОбъектСопоставления
	|			ИЗ
	|				ОбъектыСопоставления КАК ОбъектыСопоставления)
	|	И СоответствиеРеквизитовБизнесСеть.ИдентификаторРеквизитаКатегории <> """"
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	СоответствиеЗначенийРеквизитовБизнесСеть.РеквизитОбъекта КАК РеквизитОбъекта,
	|	СоответствиеЗначенийРеквизитовБизнесСеть.Значение КАК Значение,
	|	ВЫРАЗИТЬ(СоответствиеЗначенийРеквизитовБизнесСеть.ПредставлениеЗначенияРеквизитаКатегории КАК СТРОКА(255)) КАК ПредставлениеЗначенияРеквизитаКатегории
	|ПОМЕСТИТЬ ВсеЗначенияРеквизитов
	|ИЗ
	|	РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть КАК СоответствиеЗначенийРеквизитовБизнесСеть
	|ГДЕ
	|	СоответствиеЗначенийРеквизитовБизнесСеть.ОбъектСопоставления В
	|			(ВЫБРАТЬ
	|				ОбъектыСопоставления.ОбъектСопоставления
	|			ИЗ
	|				ОбъектыСопоставления КАК ОбъектыСопоставления)
	...
	|	ВсеРеквизиты.ПредставлениеРеквизитаКатегории КАК Свойства_Свойство,
	|	ВЫБОР
	|		КОГДА НЕ ВсеЗначенияРеквизитов.ПредставлениеЗначенияРеквизитаКатегории ЕСТЬ NULL
	|			ТОГДА ВсеЗначенияРеквизитов.ПредставлениеЗначенияРеквизитаКатегории
	|		ИНАЧЕ ТаблицаСвойств.Значение
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВсеЗначенияРеквизитов КАК ВсеЗначенияРеквизитов
	|		ПО (ТаблицаСвойств.Значение = ВсеЗначенияРеквизитов.Значение)
	|			И (ТаблицаСвойств.Свойство = ВсеЗначенияРеквизитов.РеквизитОбъекта)
	|ГДЕ
	|	(НЕ &ТолькоСопоставленныеСвойства
	|			ИЛИ НЕ ТаблицаСвойств.Значение ЕСТЬ NULL)";
	
	Возврат ТекстЗапроса;
КонецФункции

Функция ТекстЗапросаСвойстваРаботаСНоменклатурой()
	...
	|	СоответствиеРеквизитовРаботаСНоменклатурой.ПредставлениеРеквизитаКатегории КАК ПредставлениеРеквизитаКатегории
	|ПОМЕСТИТЬ ВсеРеквизиты
	|ИЗ
	|	РегистрСведений.СоответствиеРеквизитовРаботаСНоменклатурой КАК СоответствиеРеквизитовРаботаСНоменклатурой
	|ГДЕ
	|	СоответствиеРеквизитовРаботаСНоменклатурой.ОбъектСопоставления = &Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрСведений.СоответствиеРеквизитовРаботаСНоменклатурой КАК СоответствиеРеквизитовРаботаСНоменклатурой
	|ГДЕ
	|	СоответствиеРеквизитовРаботаСНоменклатурой.ОбъектСопоставления = &Родитель
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрСведений.СоответствиеРеквизитовРаботаСНоменклатурой КАК СоответствиеРеквизитовРаботаСНоменклатурой
	|ГДЕ
	|	СоответствиеРеквизитовРаботаСНоменклатурой.ОбъектСопоставления = &ВидНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫРАЗИТЬ(СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.ПредставлениеЗначенияРеквизитаКатегории КАК СТРОКА(255)) КАК ПредставлениеЗначенияРеквизитаКатегории
	|ПОМЕСТИТЬ ВсеЗначенияРеквизитов
	|ИЗ
	...
	|ГДЕ
	|	СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.ОбъектСопоставления = &Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.РеквизитОбъекта,
	|	СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.Значение,
	|	ВЫРАЗИТЬ(СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.ПредставлениеЗначенияРеквизитаКатегории КАК СТРОКА(255))
	|ИЗ
	|	РегистрСведений.СоответствиеЗначенийРеквизитовРаботаСНоменклатурой КАК СоответствиеЗначенийРеквизитовРаботаСНоменклатурой
	|ГДЕ
	...
	|	РегистрСведений.СоответствиеЗначенийРеквизитовРаботаСНоменклатурой КАК СоответствиеЗначенийРеквизитовРаботаСНоменклатурой
	|ГДЕ
	|	СоответствиеЗначенийРеквизитовРаботаСНоменклатурой.ОбъектСопоставления = &Родитель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВсеРеквизиты.РеквизитОбъекта КАК РеквизитОбъекта,
	|	ВЫБОР
	|		КОГДА ВсеРеквизиты.ПредставлениеРеквизитаКатегории = """"
	|			ТОГДА ПРЕДСТАВЛЕНИЕССЫЛКИ(ВсеРеквизиты.РеквизитОбъекта)
	|		ИНАЧЕ ВсеРеквизиты.ПредставлениеРеквизитаКатегории
	|	КОНЕЦ КАК Свойства_Свойство,
	|	ВЫБОР
	|		КОГДА НЕ ВсеЗначенияРеквизитов.ПредставлениеЗначенияРеквизитаКатегории ЕСТЬ NULL
	|			ТОГДА ВсеЗначенияРеквизитов.ПредставлениеЗначенияРеквизитаКатегории
	|		ИНАЧЕ ТаблицаСвойств.Значение
...
Функция ТекстЗапросаДинамическогоСпискаРаботаСНоменклатурой() 
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеНоменклатуры
	|		ПО РегистрСведенийТорговыеПредложенияТорговойПлощадки.Номенклатура = СоответствиеНоменклатуры.ОбъектСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеРодителя
	|		ПО (ДанныеНоменклатуры.Родитель = СоответствиеРодителя.ОбъектСопоставления)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеВидаНоменклатуры
	|		ПО (ДанныеНоменклатуры.ВидНоменклатуры = СоответствиеВидаНоменклатуры.ОбъектСопоставления)";
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|	СоответствиеПоНоменклатуре.ОбъектСопоставления КАК Номенклатура,
	|	NULL КАК Категория,
	|	NULL КАК Родитель,
	|	"""" КАК Наименование,
	...
	|ИЗ
	|	РегистрСведений.СоответствиеНоменклатурыБизнесСеть КАК СоответствиеПоНоменклатуре
	|ГДЕ
	|	ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	...
	|				ОтборНоменклатуры КАК ОтборНоменклатуры
	|			ГДЕ
	|				ОтборНоменклатуры.Номенклатура = СоответствиеПоНоменклатуре.ОбъектСопоставления)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(СоответствиеРеквизитовПоНоменклатуре.ФормализованныйРеквизит, ЕСТЬNULL(СоответствиеРеквизитовПоВидам.ФормализованныйРеквизит, СоответствиеРеквизитовПоИерархии.ФормализованныйРеквизит)) КАК ФормализованныйРеквизит
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовБизнесСеть КАК СоответствиеРеквизитовПоНоменклатуре
	|		ПО Свойства.Номенклатура = СоответствиеРеквизитовПоНоменклатуре.ОбъектСопоставления
	|			И Свойства.Свойство = СоответствиеРеквизитовПоНоменклатуре.РеквизитОбъекта
	|			И (СоответствиеРеквизитовПоНоменклатуре.ФормализованныйРеквизит <> ЗНАЧЕНИЕ(Перечисление.РеквизитыНоменклатурыДляВыгрузки.ПустаяСсылка))
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовБизнесСеть КАК СоответствиеРеквизитовПоВидам
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовБизнесСеть КАК СоответствиеРеквизитовПоИерархии
	|		ПО (Товары.Родитель = СоответствиеРеквизитовПоИерархии.ОбъектСопоставления)
	|			И Свойства.Свойство = СоответствиеРеквизитовПоИерархии.РеквизитОбъекта
	|			И (СоответствиеРеквизитовПоИерархии.ФормализованныйРеквизит <> ЗНАЧЕНИЕ(Перечисление.РеквизитыНоменклатурыДляВыгрузки.ПустаяСсылка))
	|ГДЕ
	|	ЕСТЬNULL(СоответствиеРеквизитовПоНоменклатуре.ФормализованныйРеквизит, ЕСТЬNULL(СоответствиеРеквизитовПоВидам.ФормализованныйРеквизит, СоответствиеРеквизитовПоИерархии.ФормализованныйРеквизит)) ЕСТЬ НЕ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Свойство
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФормализованныеРеквизиты.Номенклатура КАК Номенклатура,
	|	ФормализованныеРеквизиты.Значение КАК Значение,
	|	ФормализованныеРеквизиты.ФормализованныйРеквизит КАК ФормализованныйРеквизит
	|ПОМЕСТИТЬ ФормализованныеРеквизитыСлужебный
	|ИЗ
	|	ФормализованныеРеквизиты КАК ФормализованныеРеквизиты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(СоответствиеРеквизитовПоНоменклатуре.ИдентификаторРеквизитаКатегории, ЕСТЬNULL(СоответствиеРеквизитовПоВидам.ИдентификаторРеквизитаКатегории, СоответствиеРеквизитовПоИерархии.ИдентификаторРеквизитаКатегории)) КАК ИдентификаторРеквизитаКатегории,
	|	ЕСТЬNULL(СоответствиеЗначенийПоНоменклатуре.ИдентификаторЗначенияРеквизитаКатегории, ЕСТЬNULL(СоответствиеЗначенийПоВидам.ИдентификаторЗначенияРеквизитаКатегории, СоответствиеЗначенийПоИерархии.ИдентификаторЗначенияРеквизитаКатегории)) КАК ИдентификаторЗначения
	...
	|		ПО Свойства.Номенклатура = СоответствиеРеквизитовПоНоменклатуре.ОбъектСопоставления
	|			И Свойства.Свойство = СоответствиеРеквизитовПоНоменклатуре.РеквизитОбъекта
	|			И (СоответствиеРеквизитовПоНоменклатуре.ИдентификаторРеквизитаКатегории <> """")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовБизнесСеть КАК СоответствиеРеквизитовПоВидам
	|		ПО (Товары.ВидНоменклатуры = СоответствиеРеквизитовПоВидам.ОбъектСопоставления)
	|			И Свойства.Свойство = СоответствиеРеквизитовПоВидам.РеквизитОбъекта
	...
	|		ПО (Товары.Родитель = СоответствиеРеквизитовПоИерархии.ОбъектСопоставления)
	|			И Свойства.Свойство = СоответствиеРеквизитовПоИерархии.РеквизитОбъекта
	|			И (СоответствиеРеквизитовПоИерархии.ИдентификаторРеквизитаКатегории <> """")
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть КАК СоответствиеЗначенийПоНоменклатуре
	|		ПО Свойства.Номенклатура = СоответствиеЗначенийПоНоменклатуре.ОбъектСопоставления
	|			И Свойства.Свойство = СоответствиеЗначенийПоНоменклатуре.РеквизитОбъекта
	|			И Свойства.Значение = СоответствиеЗначенийПоНоменклатуре.Значение
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть КАК СоответствиеЗначенийПоВидам
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть КАК СоответствиеЗначенийПоИерархии
	|		ПО (Товары.Родитель = СоответствиеЗначенийПоИерархии.ОбъектСопоставления)
	|			И Свойства.Свойство = СоответствиеЗначенийПоИерархии.РеквизитОбъекта
	|			И Свойства.Значение = СоответствиеЗначенийПоИерархии.Значение
	|ГДЕ
	|	НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					ФормализованныеРеквизиты
	|				ГДЕ
	|					ФормализованныеРеквизиты.Номенклатура = Свойства.Номенклатура
	|					И ФормализованныеРеквизиты.Свойство = Свойства.Свойство)";
	
	ТаблицыДляУничтожения = Новый Структура("Товары, Категории, Свойства, ОтборНоменклатуры, ОтборНоменклатурыСлужебный, ФормализованныеРеквизиты");
...
Функция ПроверитьЗаполнениеНоменклатуры(Запрос, Настройка, ПодготовитьЗапрос = Истина)
		...
		Для ОбратныйИндекс = 1 По КоличествоЗапросов Цикл
			...
			ИначеЕсли ТипЗнч(ПроверяемыйЗапрос) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
				...
				ИначеЕсли ПроверяемыйЗапрос.ТаблицаДляПомещения = "ОтборНоменклатуры" Тогда
					...
					ПроверяемыйЗапрос.Индекс.Добавить(ПолеНоменклатура);
	...
	Пока Истина Цикл
		...
		Если Счетчик > 1 И Счетчик % КоэффициентПорцииПрогресса = 0 Тогда
...
Функция ИсключитьРеквизитыИзЗапроса(ТекстЗапроса, ИсключенныеРеквизиты)
...
Функция КоллекцияДанныхВыгрузки(ИсточникДанных, СоответствиеПолей)
	...
	КонецЦикла;
	ИсточникДанных.Сортировать(СтрСоединить(КолонкиДанных, ","));
	...
	Для каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		Для каждого АтрибутПакета Из СоответствиеПолей Цикл
			Ключ = АтрибутПакета.Представление;
			ЗначениеАтрибута = ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДанных, Ключ), СтрокаДанных[Ключ], Неопределено);
...
Функция ВыгрузитьНоменклатуру(Запрос, Параметры)
	...
	КонецЕсли;
	ТекстУсловия = ПолеНоменклатура + " > &ПараметрПорядокНоменклатура";
	...
	ИндексыОбходаРезультатов.Вставить("ФормализованныеРеквизитыСлужебный");
	...
	Для каждого ЗапросПакета Из СхемаЗапроса.ПакетЗапросов Цикл
		Если ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
			...
			Если ТекущаяТаблица = "ТоварыСлужебный" Тогда
				...
				Для каждого КолонкаЗапроса Из ЗапросПакета.Колонки Цикл
					Если Не ДанныеБазы.Свойство(КолонкаЗапроса.Псевдоним) Тогда
	...
	Пока Истина Цикл
		...
		Если КоличествоКВыгрузке = 0 Тогда
			Прервать;
		КонецЕсли;
		ПараметрПорядокНоменклатура = ТаблицаСостояния[КоличествоКВыгрузке-1].Номенклатура;
		...
		Для каждого СтрокаТаблицыТовары Из ОбходРезультатов.ТоварыСлужебный Цикл
			Если Счетчик = 0 ИЛИ Счетчик % КоличествоЗаписейВФиде = 0 Тогда
				Если ЗначениеЗаполнено(КорневойЭлемент.id) Тогда
					ОписаниеПакетаДанных = Новый Структура("НоменклатураПакета, ПакетДанных", НоменклатураПакета, КорневойЭлемент);
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.ФормализованныеРеквизитыСлужебный) Тогда
				СтрокиВложеннойТаблицы = ОбходРезультатов.ФормализованныеРеквизитыСлужебный.НайтиСтроки(ПоискБезХарактеристики);
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.СвойстваСлужебный) Тогда
				...
				ТаблицаДанных = ОбходРезультатов.СвойстваСлужебный.Скопировать(СтрокиВложеннойТаблицы);
				КолонкиДанных = Новый Массив;
				Для каждого Колонка Из ТаблицаДанных.Колонки Цикл
					КолонкиДанных.Добавить(Колонка.Имя);
				КонецЦикла;
				ТаблицаДанных.Сортировать(СтрСоединить(КолонкиДанных, ","));
				...
				Для каждого СтрокаДанных Из ТаблицаДанных Цикл
					...
					Если ЗначениеЗаполнено(СтрокаДанных.ИдентификаторРеквизитаКатегории) Тогда
						...
						Если ЗначениеЗаполнено(СтрокаДанных.ИдентификаторЗначения) Тогда
							ЭлементСсылкаНоменклатура.Вставить("value", СтрокаДанных.ИдентификаторЗначения);
						КонецЕсли;
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.КатегорииСлужебный) Тогда
				...
				ТаблицаДанных          = ОбходРезультатов.КатегорииСлужебный.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Штрихкоды) Тогда
				...
				ТаблицаДанных          = ОбходРезультатов.Штрихкоды.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Упаковки) Тогда
				...
				СтрокиВложеннойТаблицы = ОбходРезультатов.Упаковки.НайтиСтроки(ПоискБезХарактеристики);
				ТаблицаДанных          = ОбходРезультатов.Упаковки.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Классификаторы) Тогда
				...
				СтрокиВложеннойТаблицы = ОбходРезультатов.Классификаторы.НайтиСтроки(ПоискБезХарактеристики);
				ТаблицаДанных          = ОбходРезультатов.Классификаторы.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
					Если ЭлементНоменклатура.Свойство("classifiers") Тогда
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Изображения) Тогда
				...
				СтрокиВложеннойТаблицы = ОбходРезультатов.Изображения.НайтиСтроки(ПоискБезХарактеристики);
				ТаблицаДанных          = ОбходРезультатов.Изображения.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Вложения) Тогда
				...
				СтрокиВложеннойТаблицы = ОбходРезультатов.Вложения.НайтиСтроки(ПоискБезХарактеристики);
				ТаблицаДанных          = ОбходРезультатов.Вложения.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.Сертификаты) Тогда
				...
				СтрокиВложеннойТаблицы = ОбходРезультатов.Сертификаты.НайтиСтроки(ПоискБезХарактеристики);
				ТаблицаДанных          = ОбходРезультатов.Сертификаты.Скопировать(СтрокиВложеннойТаблицы);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(ТаблицаДанных, ДанныеПакета);
				Если КоллекцияДанных.Количество() Тогда
					Для каждого ЭлементКоллекции Из КоллекцияДанных Цикл
						...
						ВложенияСертификатов = ОбходРезультатов.ВложенияСертификатов.Скопировать(ВложенияСертификатов);
...
Функция ШтрихкодыНоменклатуры()
	...
	ШтрихкодыНоменклатуры.Колонки.Добавить("Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(0));
...
Функция СопоставленныеРеквизиты()
	...
	Для каждого ЗапросПакета Из СхемаЗапроса.ПакетЗапросов Цикл
		Если ЗапросПакета.ТаблицаДляПомещения = "Товары" Тогда
			Для каждого Реквизит Из РеквизитыНоменклатуры() Цикл
				...
				Если КолонкаЗапроса = Неопределено 
					ИЛИ КолонкаЗапроса.Поля[0] = Новый ВыражениеСхемыЗапроса("NULL") Тогда
					Продолжить;
				КонецЕсли;
				СопоставленныеРеквизиты.Добавить(Реквизит.Ключ);
			КонецЦикла;
		ИначеЕсли ЗапросПакета.ТаблицаДляПомещения = "Сертификаты" Тогда
			СопоставленныеРеквизиты.Добавить("СертификатыНомер");
			СопоставленныеРеквизиты.Добавить("СертификатыДатаНачалаСрокаДействия");
		ИначеЕсли ЗапросПакета.ТаблицаДляПомещения = "Классификаторы" Тогда
