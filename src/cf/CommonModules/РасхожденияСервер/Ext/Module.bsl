...
Процедура ЗаполнитьПоОснованиям(Форма, ВызыватьУправлениеДоступностью = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Если Форма.Объект.ОснованиеДляСоставленияАкта <> "Входящий электронный документ" Тогда
				Форма.Объект.Товары.Загрузить(РезультатЗапроса[0].Выгрузить());
			Иначе
				ДанныеЗапроса = РезультатЗапроса[0].Выбрать();
				Если ДанныеЗапроса.Следующий() Тогда
					Реализация = ДанныеЗапроса.Реализация;
					Склад = ДанныеЗапроса.Склад;
					Для Каждого Строка Из Форма.Объект.Товары Цикл
						Строка.Реализация = Реализация;
						Строка.Склад = Склад;							
					КонецЦикла
				КонецЕсли;
			КонецЕсли;
