	...
	ИначеЕсли ТипЗнч(УпаковкаНоменклатура) = Тип("Структура") Тогда
		...
		Если УпаковкаНоменклатура.Свойство("НужноОкруглять") Тогда
			Если (ТипЗнч(ТекущаяСтрока) = Тип("Структура") 
				Или ТипЗнч(ТекущаяСтрока) = Тип("ДанныеФормыЭлементКоллекции"))
				И ТекущаяСтрока.Свойство("НужноОкруглять")
				#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
					Или ТипЗнч(ТекущаяСтрока) = Тип("СтрокаТаблицыЗначений")
					И ТекущаяСтрока.Владелец().Колонки.Найти("НужноОкруглять") <> Неопределено 
				#КонецЕсли
				Тогда
