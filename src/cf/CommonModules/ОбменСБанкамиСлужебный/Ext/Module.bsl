...
Процедура СформироватьПлатежноеПоручение(
	...
	ПроверитьВозможностьПередачиКодаВидаДохода(
...
Процедура ПроверитьЗаполнениеРеквизитовПлатежногоПоручения(СсылкаНаОбъект, ДеревоДокумента, Ошибки)
	...
	Если ЗначениеЗаполнено(ПоказательКБК) Тогда
		...
		КонецЕсли;
		Если СтрДлина(ПоказательКБК) <> 20 Тогда
			ТекстОшибки = НСтр("ru = 'Длина КБК должна быть 20 символов'");
			СтрокаДерева = ДеревоДокумента.Строки.Найти("ПлатежиВБюджет.ПоказательКБК", "ПолныйПуть", Истина);
			ЭлектронноеВзаимодействиеСлужебный.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтрокаДерева, ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВозможностьПередачиКодаВидаДохода(
	СсылкаНаОбъект,
	ДеревоДокумента,
	ПрограммаБанка,
	ВерсияФормата,
...
Функция ТаблицаВидовЭДИПрограмм()
	...
	// ====== Запрос-зонд =============.
	ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросЗонд;
...
Функция ДайджестВыпискиСбербанк(ДеревоРазбора, СтрокаОбъекта)
	...
	Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
		...
		КодВидаДохода = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "КодВидаДохода");
		ДанныеЗаполненияСтроки.Добавить(КодВидаДохода, "codeOfRevenueType");
