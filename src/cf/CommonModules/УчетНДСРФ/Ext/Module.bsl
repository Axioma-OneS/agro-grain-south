...
Процедура ПолучитьРазрезыВычетаНДС(ПараметрыРасчета) Экспорт
	...
	|			ТОГДА ЛОЖЬ
	|		КОГДА РазрезыВычетаНДС.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров)
	|			И  ЕСТЬNULL(ДанныеПервичныхДокументовСчетФактура.ДатаРегистратора, ДАТАВРЕМЯ(1,1,1)) >= &ДатаНачалаДействия150ФЗ 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПоВидуДеятельностиОсобыйПорядокВычетаНДС,
	|	
	|	ВЫБОР
	...
	Если ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода < Константы.ДатаНачалаПримененияСводныхСправок.Получить()
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Сводные справки не применяются';
				|en = 'Сводные справки не применяются'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ИначеЕсли ЗакрытиеМесяцаСервер.ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, Ложь, "ОрганизацииОСНО")
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			...
			НСтр("ru = 'Необходимо сформировать сводные справки';
				|en = 'Необходимо сформировать сводные справки'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Иначе
...
Процедура РассчитатьЗачетАвансовПолученных(ПараметрыРасчета)
	...
	|	РасчетыСКлиентами.РасчетныйДокумент КАК РасчетныйДокумент,
	|	СУММА(РасчетыСКлиентами.ВидЦенностиЗнак) КАК ВидЦенностиЗнак,
	|	СУММА(РасчетыСКлиентами.СуммаЗачета) КАК СуммаЗачета,
	|	СУММА(РасчетыСКлиентами.СуммаЗачетаУпр) КАК СуммаЗачетаУпр
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	РасчетыСКлиентами.НаправлениеДеятельности,
	|	РасчетыСКлиентами.Контрагент,
	|	РасчетыСКлиентами.ДокументОплаты,
	|	РасчетыСКлиентами.РасчетныйДокумент
	|
	|ИМЕЮЩИЕ
	|	СУММА(РасчетыСКлиентами.СуммаЗачета) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура РассчитатьЗачетАвансовВыданных(ПараметрыРасчета)
	...
	|	РасчетыСПоставщиками.РасчетныйДокумент КАК ДокументОплаты,
	|	РасчетыСПоставщиками.РасчетныйДокумент КАК РасчетныйДокумент,
	|	АвансыВыданные.НалогообложениеНДС КАК НалогообложениеНДС,
	|	СУММА(РасчетыСПоставщиками.ПредоплатаРегл * ВЫБОР
	|			КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК СуммаЗачета,
	|	СУММА(РасчетыСПоставщиками.ПредоплатаУпр * ВЫБОР
	|			КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК СуммаЗачетаУпр
	|ПОМЕСТИТЬ вт_РасчетСПоставщикамиДляЗачетаАванса
	|ИЗ
	...
	|	ПО
	|		РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Контрагент = АвансыВыданные.Контрагент
	|		И РасчетыСПоставщиками.РасчетныйДокумент = АвансыВыданные.ДокументОплаты
	|ГДЕ
	|	НЕ &НоваяАрхитектураВзаиморасчетов
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	РасчетыСПоставщиками.Контрагент КАК Покупатель,
	|	РасчетыСПоставщиками.ДокументОплаты КАК СчетФактура,
	|	ВЫБОР КОГДА РасчетыСПоставщиками.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя)
	...
	|	РасчетыСПоставщиками.СуммаЗачета КАК СуммаЗачета,
	|	РасчетыСПоставщиками.СуммаЗачетаУпр КАК СуммаЗачетаУпр,
	|	Значение(Перечисление.ХозяйственныеОперации.ВосстановлениеНДССВыданногоАванса) КАК ХозяйственнаяОперация,
	|	ВЫБОР КОГДА РасчетыСПоставщиками.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|		ТОГДА ""21""
	|		ИНАЧЕ ""43""
	|	КОНЕЦ КАК КодВидаОперации
	|ПОМЕСТИТЬ ВтЗачетАвансов
	|ИЗ
	|	вт_РасчетСПоставщикамиДляЗачетаАванса КАК РасчетыСПоставщиками
	|
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ вт_РасчетСПоставщикамиДляЗачетаАванса
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПолучитьДокументыОплатыЭлектронныхУслуг(ПараметрыРасчета)
	...
	|	Оплаты.ДатаДокументаОплаты КАК ДатаДокументаОплаты,
	|	Оплаты.НомерДокументаОплаты КАК НомерДокументаОплаты,
	|	СУММА(Оплаты.СуммаОплаты) КАК СуммаОплаты,
	|	МАКСИМУМ(Оплаты.СуммаПриобретения) КАК СуммаПриобретения
	...
	|		ПриобретенияЭлектронныхУслуг.Сумма КАК СуммаПриобретения,
	...
	|		ПО 
	|			Расчеты.АналитикаУчетаПоПартнерам = ПриобретенияЭлектронныхУслуг.АналитикаУчетаПоПартнерам
	|			И Расчеты.ДокументРегистратор = ПриобретенияЭлектронныхУслуг.СчетФактура
	|			
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО
	|			Расчеты.АналитикаУчетаПоПартнерам.Организация = ДанныеПервичныхДокументов.Организация
	|			И Расчеты.РасчетныйДокумент = ДанныеПервичныхДокументов.Документ
	|
	|	ГДЕ
	...
	|		ПО 
	|			Расчеты.АналитикаУчетаПоПартнерам = ПриобретенияЭлектронныхУслуг.АналитикаУчетаПоПартнерам
	|			И Расчеты.ОбъектРасчетов = ПриобретенияЭлектронныхУслуг.СчетФактура
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО
	|			Расчеты.АналитикаУчетаПоПартнерам.Организация = ДанныеПервичныхДокументов.Организация
	|			И Расчеты.ДокументРегистратор = ДанныеПервичныхДокументов.Документ
	|	ГДЕ
	...
	|		ПО 
	|			Расчеты.АналитикаУчетаПоПартнерам = ПриобретенияЭлектронныхУслуг.АналитикаУчетаПоПартнерам
	|			И Расчеты.РасчетныйДокумент = ПриобретенияЭлектронныхУслуг.СчетФактура
	|			
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО
	...
	|		ПО
	|			Расчеты.АналитикаУчетаПоПартнерам = ПриобретенияЭлектронныхУслуг.АналитикаУчетаПоПартнерам
	|			И Расчеты.Регистратор = ПриобретенияЭлектронныхУслуг.СчетФактура
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО
	...
	|		ПО
	|			Расчеты.АналитикаУчетаПоПартнерам = ПриобретенияЭлектронныхУслуг.АналитикаУчетаПоПартнерам
	|			И Расчеты.ЗаказПоставщику = ПриобретенияЭлектронныхУслуг.СчетФактура
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО
	|			Расчеты.АналитикаУчетаПоПартнерам.Организация = ДанныеПервичныхДокументов.Организация
	|			И Расчеты.Регистратор = ДанныеПервичныхДокументов.Документ
	|	ГДЕ
	...
	|) КАК Оплаты
	|ГДЕ
	|	Оплаты.СчетФактура <> Оплаты.ДокументОплаты
	|
	|СГРУППИРОВАТЬ ПО
	|	Оплаты.Организация,
	|	Оплаты.СчетФактура,
	|	Оплаты.ДокументОплаты,
	|	Оплаты.ДатаДокументаОплаты,
	|	Оплаты.НомерДокументаОплаты
	|";
	Запрос.УстановитьПараметр("НачалоПериода",     ПараметрыРасчета.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода",  ПараметрыРасчета.КонецПериода);
	Запрос.УстановитьПараметр("МассивОрганизаций", ПараметрыРасчета.МассивОрганизаций);
