...
Процедура СформироватьПредставлениеТорговогоПредложения(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Если ЗначениеЗаполнено(Результат.МинимальнаяСуммаЗаказа) Тогда
		ПараметрыВывода.МинимальнаяСуммаЗаказа = Формат(Результат.МинимальнаяСуммаЗаказа, "ЧЦ=15; ЧДЦ=2") + " " + Валюта;
	КонецЕсли;
...
Процедура ВывестиАдресаДоставки(Макет, ТабличныйДокумент, КоллекцияАдресов, ЗаголовокРаздела, ИмяПоляПредставлением)
	...
	Для каждого ЭлементКоллекции Из КоллекцияАдресов Цикл
		...
		ОбластьАдресаДанные.Параметры.АдресДоставки = ЭлементКоллекции[ИмяПоляПредставлением];
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|	СоответствиеРеквизитовБизнесСеть.ТипРеквизитаКатегории КАК ТипРеквизитаКатегории,
	|	СоответствиеРеквизитовБизнесСеть.МинимальноеЗначение КАК МинимальноеЗначение,
	|	СоответствиеРеквизитовБизнесСеть.МаксимальноеЗначение КАК МаксимальноеЗначение
	...
	Если ЕстьПодсистемаРаботаСНоменклатурой Тогда
		МодульРаботаСНоменклатурой = ОбщегоНазначения.ОбщийМодуль("РаботаСНоменклатурой");
		ТекстСопоставления = МодульРаботаСНоменклатурой.ТекстЗапросаСоответствияДляПубликацииТорговыхПредложений();
...
Процедура ПроверитьЗаписиТорговыеПредложенияТорговойПлощадки(Знач Отбор, Знач АдресРезультата = Неопределено) Экспорт 
	...
	Пока ВыборкаПрайсЛисты.Следующий() Цикл
		...
		Для каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизитыКонтакты Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если ПустаяСтрока(ВыборкаКонтакты.Телефон)
				ИЛИ СтрДлина(ВыборкаКонтакты.Телефон) < 11 ИЛИ СтрДлина(ВыборкаКонтакты.Телефон) > 100 Тогда
...
Процедура ЗаписатьДанныеРегионовВСостояниеСинхронизации(
	...
	ЗаписьРедактирования.РегионыДоставки = РегионыДоставки;
	ЗаписьРедактирования.РегионыСамовывоза = АдресаСамовывоза;
