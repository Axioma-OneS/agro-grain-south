...
Функция РассчитатьВсеВПопыткеИсключении(ПараметрыЗапуска, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	Попытка
		...
		ИменаУдаляемыхРасширений = ПроверитьЗапрещенныеРасширенияКонфигурации();
		Если ЗначениеЗаполнено(ИменаУдаляемыхРасширений) Тогда
			ВызватьИсключение ИменаУдаляемыхРасширений;
		КонецЕсли;
...
Функция ПроверитьЗапрещенныеРасширенияКонфигурации() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЗапрещенныеРасширения = Новый Структура;
	ЗапрещенныеРасширения.Вставить("EF_00_00298005"); // содержит ошибку 00-00303344
	
	ИменаУдаляемыхРасширений = "";
	
	Попытка
		Для Каждого ТекущееРасширение Из РасширенияКонфигурации.Получить() Цикл
			
			Если ЗапрещенныеРасширения.Свойство(ТекущееРасширение.Имя) И ТекущееРасширение.Активно Тогда
				ИменаУдаляемыхРасширений = ИменаУдаляемыхРасширений + ?(ИменаУдаляемыхРасширений = "", "", " ,") + ТекущееРасширение.Имя;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИменаУдаляемыхРасширений) Тогда
		ИменаУдаляемыхРасширений = НСтр("ru='Необходимо удалить расширения конфигурации'") + ": " + ИменаУдаляемыхРасширений;
	КонецЕсли;
	
	Возврат ИменаУдаляемыхРасширений;
