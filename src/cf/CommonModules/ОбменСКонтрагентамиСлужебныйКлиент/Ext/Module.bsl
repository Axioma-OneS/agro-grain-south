...
Процедура ОбработатьДокументПослеУстраненияОшибок(Результат, Контекст) Экспорт
	...
	Если (Не ЗначениеЗаполнено(Результат.Действие) Или Результат.Действие = "Сформировать")
		...
		Если ТипЗнч(Контекст) = Тип("Структура")
			...
			Результат.Вставить("ТекущиеНастройкиОбмена", Контекст.ДопПараметры.НастройкиОбмена.Получить(МассивСсылокНаОбъекты[0]));
...
Процедура ПроверитьФормированиеЭД(СсылкаНаОбъект, ДополнительныеПараметры = Неопределено)
	...
	ТекущиеНастройкиОбмена = Новый Структура;
	
	Если ДополнительныеПараметры.Свойство("ТекущиеНастройкиОбмена") Тогда
		ТекущиеНастройкиОбмена = ДополнительныеПараметры.ТекущиеНастройкиОбмена;
	КонецЕсли;
	
	Результат = ОбменСКонтрагентамиСлужебныйВызовСервера.СформироватьЭДПоОснованиюИСохранитьВФайл(
		СсылкаНаОбъект, ТекущиеНастройкиОбмена, ДополнительныеПараметры);
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Успех = Истина Тогда
			...
			ПараметрыФормы.Вставить("ТекущиеНастройкиОбмена" , ТекущиеНастройкиОбмена);
...
Процедура ОбработатьСозданиеЗаявкиПриПолученииНовогоИдентификатораТакском(Результат, ДополнительныеПараметры) Экспорт
	...
	Ошибки = ОшибкиВДанныхЗаявкиТакском(ДанныеЗаявки);
...
Функция ОшибкиВДанныхЗаявкиТакском(Знач ДанныеЗаявки) Экспорт
	...
	Если ПустаяСтрока(Телефон) Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""Телефон""'"));
	КонецЕсли;
	
	// Проверка адреса
	
	Если ПустаяСтрока(КодРегиона) Тогда
		Ошибки.Добавить(НСтр("ru ='Не указан ""Код региона"" (Код региона нужно указать в форме ""Адрес участника обмена ЭД"")'"));
	КонецЕсли;
	
	Если ПустаяСтрока(ИНН) Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""ИНН""'"));
	КонецЕсли;
	
	Если ЮрФизЛицо <> "ЮрЛицо" И ЮрФизЛицо <> "ФизЛицо" Тогда
		Ошибки.Добавить(НСтр("ru ='Не выбран тип организации (юридическое или физическое лицо)'"));
	КонецЕсли;
	
	Если ПустаяСтрока(КПП) И ЮрФизЛицо = "ЮрЛицо" Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""КПП""'"));
	КонецЕсли;
	
	Если ПустаяСтрока(ОГРН) И ЮрФизЛицо = "ЮрЛицо" Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""ОГРН""'"));
	КонецЕсли;
	
	Если ПустаяСтрока(КодНалоговогоОргана) Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""Код налогового органа""'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Фамилия) Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""Фамилия""'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Имя) Тогда
		Ошибки.Добавить(НСтр("ru ='Не заполнено поле ""Имя""'"));
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Телефон) Тогда
		Если СтрДлина(Телефон) > 20 Тогда
			Ошибки.Добавить(НСтр("ru ='""Телефон"" должен содержать не более 20 символов'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(КодРегиона) Тогда
		Если КодРегиона <> "0" Тогда
			...
			Если КодРегионаЧисло = 0 Тогда
				Ошибки.Добавить(НСтр("ru ='В ""Коде региона"" использованы недопустимые символы'"));
			Иначе
				Если СтрДлина(СокрЛП(КодРегиона)) <> 2 Тогда
					Ошибки.Добавить(НСтр("ru ='""Код региона"" должен содержать 2 цифры'"));
				Иначе
					Если КодРегионаЧисло > 99 ИЛИ КодРегионаЧисло < 1 Тогда
						Ошибки.Добавить(НСтр("ru ='""Код региона"" должен быть от 01 до 99'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
	...
	Если НЕ ПустаяСтрока(КПП) Тогда
		Если КПП <> "0" Тогда
			...
			Если КППЧисло = 0 Тогда
				Ошибки.Добавить(НСтр("ru ='В ""КПП"" использованы недопустимые символы'"));
			Иначе
				Если СтрДлина(СокрЛП(КПП)) <> 9 Тогда
					Ошибки.Добавить(НСтр("ru ='""КПП"" должен содержать 9 цифр'"));
				КонецЕсли;
			КонецЕсли;
		Иначе
			Ошибки.Добавить(НСтр("ru ='""КПП"" должен содержать 9 цифр'"));
	...
	Если НЕ ПустаяСтрока(ОГРН) Тогда
		Если ОГРН <> "0" Тогда
			...
			Если ОГРНЧисло = 0 Тогда
				Ошибки.Добавить(НСтр("ru ='В ""ОГРН"" использованы недопустимые символы'"));
			Иначе
				Если СтрДлина(СокрЛП(ОГРН)) <> 13  И ЮрФизЛицо = "ЮрЛицо" Тогда
					Ошибки.Добавить(НСтр("ru ='""ОГРН"" должен содержать 13 цифр'"));
				ИначеЕсли СтрДлина(СокрЛП(ОГРН)) <> 15  И ЮрФизЛицо = "ФизЛицо" Тогда
					Ошибки.Добавить(НСтр("ru ='""ОГРН"" должен содержать 15 цифр'"));
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если СтрДлина(СокрЛП(ОГРН)) <> 13  И ЮрФизЛицо = "ЮрЛицо" Тогда
	...
	Если НЕ ПустаяСтрока(КодНалоговогоОргана) Тогда
		Если КодНалоговогоОргана <> "0" Тогда
			Если ОГРНЧисло = 0 Тогда
				Ошибки.Добавить(НСтр("ru ='В ""Коде налогового органа"" использованы недопустимые символы'"));
			Иначе
				Если СтрДлина(СокрЛП(КодНалоговогоОргана)) <> 4 Тогда
					Ошибки.Добавить(НСтр("ru ='""Код налогового органа"" должен содержать 4 цифры'"));
				КонецЕсли;
			КонецЕсли;
		Иначе
			Ошибки.Добавить(НСтр("ru ='""Код налогового органа"" должен содержать 4 цифры'"));
...
Процедура СообщитьОбОшибкахВДанныхЗаявкиТакском(Знач Ошибки) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Обнаружены ошибки в заявке оператору ""Такском"":'") + Символы.ПС
