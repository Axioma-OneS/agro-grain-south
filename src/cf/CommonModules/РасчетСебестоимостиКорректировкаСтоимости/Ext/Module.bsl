...
Функция ПроверитьНеобходимостьПерепроведенияДокументов_00303344(ПараметрыОбработчика) Экспорт
	
	ПараметрыРасчета = ПараметрыОбработчика.ПараметрыРасчета;
	
	Результат = Новый Структура;
	Результат.Вставить("ТекстЗакрытиеМесяца", "");
	Результат.Вставить("ТекстРасширение", "");
	Результат.Вставить("ТекстПерепроведение", "");
	...
	КоличествоЗаданий =
		?(ОбщегоНазначения.ИнформационнаяБазаФайловая(),
			1,
			ЗначенияПараметров.МаксимальноеКоличествоФЗЗаписи);
	
	Если НЕ ПолучитьФункциональнуюОпцию("УправлениеПредприятием")
	 ИЛИ КоличествоЗаданий < 2 Тогда
		Возврат Результат; // проблема есть только в ERP в клиент-серверном варианте
	КонецЕсли;
	
	Результат.ТекстРасширение = РасчетСебестоимости.ПроверитьЗапрещенныеРасширенияКонфигурации();
	
	Если ЗначениеЗаполнено(Результат.ТекстРасширение) Тогда
		
		Результат.ТекстЗакрытиеМесяца = НСтр("ru='Перед выполнением операций закрытия месяца необходимо удалить некоторые расширение конфигурации'");
		
		ЗаписьЖурналаРегистрации(
			ЗакрытиеМесяцаСервер.ИмяСобытияЖурналаРегистрации(ОбщегоНазначения.ИмяЗначенияПеречисления(ПараметрыОбработчика.ДанныеЭтапа.Код)),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			Результат.ТекстРасширение);
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Т.ПериодРасчета КАК ПериодРасчета
	|ПОМЕСТИТЬ ВТПериоды
	|ИЗ
	|	(ВЫБРАТЬ
	|		Т.ПериодРасчета КАК ПериодРасчета,
	|		ИСТИНА КАК ЕстьПроблемныйРасчет,
	|		ЛОЖЬ КАК ЕстьРасчетСИсправлением
	|	ИЗ
	|		РегистрСведений.ПротоколыРасчетаПартийИСебестоимости КАК Т
	|	ГДЕ
	|		Т.Период >= &ПериодНачалаПатча
	|		И Т.Период <= &ПериодОкончанияПатча
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		РегистрСведений.ПротоколыРасчетаПартийИСебестоимости КАК Т
	|	ГДЕ
	|		Т.Период >= &ПериодОкончанияПатча) КАК Т
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.ПериодРасчета
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(Т.ЕстьПроблемныйРасчет) = ИСТИНА
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПериодРасчета
	|;
	...
	|		ПО Т.ПериодРасчета = ВТПериоды.ПериодРасчета
	|ГДЕ
	|	ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	...
	|			ГДЕ
	|				НАЧАЛОПЕРИОДА(ВТПериоды.ПериодРасчета, МЕСЯЦ) = &ПериодРасчета)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПериодРасчета";
	
	Запрос.УстановитьПараметр("ПериодНачалаПатча", 	  Дата(2020,06,04));
	Запрос.УстановитьПараметр("ПериодОкончанияПатча", ?(ЗначениеЗаполнено(Результат.ТекстРасширение), ТекущаяДатаСеанса(), КонецДня(Дата(2020,06,14))));
	Запрос.УстановитьПараметр("ПериодРасчета", 	  	  НачалоМесяца(ПараметрыРасчета.ПериодРегистрации));
	...
	Пока Выборка.Следующий() Цикл
		...
		ПериодПротокола = НачалоМесяца(Выборка.ПериодРасчета);
		Если МассивПериодов.Найти(ПериодПротокола) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Попытка
			Протокол = РегистрыСведений.ПротоколыРасчетаПартийИСебестоимости.ПолучитьТекстПротокола(Выборка.Идентификатор, Выборка.Период);
		Исключение
			Продолжить;
		КонецПопытки;
		
		ТекстПротокола = Протокол.ПолучитьТекст();
		Если СтрНайти(ТекстПротокола, ": 2.4.11.") = 0 // ошибка только в 2.4.11
		 ИЛИ СтрНайти(ТекстПротокола, НСтр("ru='Активные расширения'")) > 0 Тогда // если в протоколе есть такая строка, то расчет выполнялся на сборке без ошибки
			Продолжить;
		КонецЕсли;
		
		МассивПериодов.Добавить(ПериодПротокола);
		СтрокаПериоды = СтрокаПериоды + ?(СтрокаПериоды = "", "", ", ") + РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПериодПротокола);
	...
	ОрганизацииДляПерепроведения = Обработки.ПерепроведениеДокументовПоРегистрамСебестоимости.ОрганизацииДляПерепроведения(
		НачалоМесяца(ПараметрыРасчета.ПериодРегистрации),
		ПараметрыРасчета.МассивОрганизаций);
	
	Если ОрганизацииДляПерепроведения.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Результат.ТекстРасширение) Тогда
		Результат.ТекстЗакрытиеМесяца = НСтр("ru='Перед выполнением операций закрытия месяца необходимо выполнить дополнительные действия'");
	Иначе
		Результат.ТекстЗакрытиеМесяца = НСтр("ru='Перед выполнением операций закрытия месяца необходимо перепровести документы'");
	КонецЕсли;
	
	Результат.ТекстПерепроведение = НСтр("ru='В результате ошибки 00-00303344 в этой информационной базе необходимо перепроведение документов за периоды: %1.
		|После этого необходимо повторно выполнить операции закрытия месяца за эти периоды.
		|Если вы уже выполнили данные действия после получения этого сообщения, то сообщение можно проигнорировать.'");
	
	Результат.ТекстПерепроведение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ТекстПерепроведение, СтрокаПериоды);
...
Процедура СписатьОшибкиОкругленияРасчетаСебестоимости(ПараметрыРасчета)
	...
	|	(ВЫБОР
	|		КОГДА Себестоимость.ПостояннаяРазница МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0 ТОГДА Себестоимость.ПостояннаяРазница
	|		ИНАЧЕ 0 КОНЕЦ) 							КАК ПостояннаяРазница,
	|	(ВЫБОР
	|		КОГДА Себестоимость.ВременнаяРазница МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0 ТОГДА Себестоимость.ВременнаяРазница
	...
	|		ИЛИ Себестоимость.СтоимостьБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.СтоимостьБезНДС <> 0
	|		ИЛИ Себестоимость.ДопРасходы МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ДопРасходы <> 0
	|		ИЛИ Себестоимость.ДопРасходыБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ДопРасходыБезНДС <> 0
	|		ИЛИ Себестоимость.СтоимостьРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.СтоимостьРегл <> 0
	|		ИЛИ Себестоимость.СтоимостьУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.СтоимостьУпр <> 0
	|		ИЛИ Себестоимость.СтоимостьЗабалансовая МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.СтоимостьЗабалансовая <> 0
	|		ИЛИ Себестоимость.Трудозатраты МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.Трудозатраты <> 0
	|		ИЛИ Себестоимость.ПостатейныеПостоянныеСНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПостоянныеСНДС <> 0
	|		ИЛИ Себестоимость.ПостатейныеПостоянныеБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПостоянныеБезНДС <> 0
	|		ИЛИ Себестоимость.ПостатейныеПеременныеСНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПеременныеСНДС <> 0
	|		ИЛИ Себестоимость.ПостатейныеПеременныеБезНДС МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПеременныеБезНДС <> 0
	|		ИЛИ Себестоимость.СтоимостьЗабалансоваяРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.СтоимостьЗабалансоваяРегл <> 0
	|		ИЛИ Себестоимость.ДопРасходыРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.ДопРасходыРегл <> 0
	|		ИЛИ Себестоимость.ТрудозатратыРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.ТрудозатратыРегл <> 0
	|		ИЛИ Себестоимость.ПостатейныеПостоянныеРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.ПостатейныеПостоянныеРегл <> 0
	|		ИЛИ Себестоимость.ПостатейныеПеременныеРегл МЕЖДУ -&ЗначениеПогрешностиСебестоимостьРегл И 0
	|			И Себестоимость.ПостатейныеПеременныеРегл <> 0
	|		ИЛИ Себестоимость.СтоимостьУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.СтоимостьУпр <> 0
	|		ИЛИ Себестоимость.ДопРасходыУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ДопРасходыУпр <> 0
	|		ИЛИ Себестоимость.ТрудозатратыУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ТрудозатратыУпр <> 0
	|		ИЛИ Себестоимость.ПостатейныеПостоянныеУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПостоянныеУпр <> 0
	|		ИЛИ Себестоимость.ПостатейныеПеременныеУпр МЕЖДУ -&ЗначениеПогрешностиСебестоимостьУпр И 0
	|			И Себестоимость.ПостатейныеПеременныеУпр <> 0)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|////////////////////////////////////////////////////////////////////////////////
	|
