...
Функция НастройкиФормы(Форма) Экспорт
	...
	// Карло... 16.03.21
	//
	ИначеЕсли Форма = ПредопределенноеЗначение("Перечисление.ФормыОткрываемыеПриНачалеРаботыСистемы.АРМ_Закупки") Тогда 
		СтруктураПараметров.Вставить("ИмяЗапускаемойФормы", "Обработка.АРМ_Закупки.Форма.Форма");
		СтруктураПараметров.Вставить("Роль",                "ИспользоватьАРМ_Закупки");
		СтруктураПараметров.Вставить("ПараметрЗапуска",     "PurchaseWorkplace");
