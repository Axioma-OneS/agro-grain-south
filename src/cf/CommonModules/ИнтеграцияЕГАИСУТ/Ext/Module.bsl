...
Процедура РассчитатьСтатусОформленияАктПостановкиНаБалансЕГАИС(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.АктПостановкиНаБалансЕГАИС")
		...
		Если ТипЗнч(ИсточникСсылка) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
			
			ЗаполнитьРеквизитыРасчетаСтатусаоформленияПоТТНВходящейЕГАИС(ИсточникСсылка, ЗначенияРеквизитов, ЗаписьТребуется);
			
		ИначеЕсли ЗначениеЗаполнено(ИсточникСсылка) Тогда
			...
			ЗаписьТребуется = ТребуетсяОформлениеДокументовЕГАИС(ИсточникСсылка, Метаданные.Документы.АктПостановкиНаБалансЕГАИС);
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ТТНВходящаяЕГАИС") Тогда
		...
		ЗаполнитьРеквизитыРасчетаСтатусаоформленияПоОбъектуТТНВходящейЕГАИС(Источник, ЗначенияРеквизитов, ЗаписьТребуется);
...
Функция ЗапросСтатусаОформленияАктПостановкиНаБалансЕГАИС(ДокументОснование) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
		...
		Возврат ЗапросСтатусаОформленияПередачаВРегистр3ЕГАИСНаОснованииТТНВходящаяЕГАИС();
...
// См. ИнтеграцияЕГАИСПереопределяемый.РассчитатьСтатусОформленияЧекЕГАИСВозврат
...
// См. ИнтеграцияЕГАИСПереопределяемый.ЗапросСтатусаОформленияЧекЕГАИСВозврат
...
#Область ОбщиеПроцедурыИФункцииРасчетаСтатусаОформления

Процедура ЗаполнитьРеквизитыРасчетаСтатусаоформленияПоТТНВходящейЕГАИС(ТТНВходящая, ЗначенияРеквизитов, ЗаписьТребуется)
	...
	|	МИНИМУМ(ТаблицаТовары.Справка2.Поштучная) КАК ВсеСправки2Поштучные
	|ИЗ
	...
	|	Таблица.Грузополучатель,
	|	СтатусыДокументовЕГАИС.Статус");
	
	Запрос.УстановитьПараметр("Ссылка", ТТНВходящая);
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	...
	ЗаписьТребуется = ИнтеграцияЕГАИС.ИспользоватьРегистр2(Выборка.Грузополучатель)
		...
		И НЕ Выборка.ВсеСправки2Поштучные;

КонецПроцедуры

Процедура ЗаполнитьРеквизитыРасчетаСтатусаоформленияПоОбъектуТТНВходящейЕГАИС(ТТНВходящаяОбъект, ЗначенияРеквизитов, ЗаписьТребуется)
	...
	Запрос.УстановитьПараметр("Ссылка", ТТНВходящаяОбъект.Ссылка);
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	
	ЗаписьТребуется = ИнтеграцияЕГАИС.ИспользоватьРегистр2(ТТНВходящаяОбъект.Грузополучатель)
		И (Выборка.Статус = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.Подтвержден
			Или Выборка.Статус = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.ПодтвержденСРасхождениями);
	
	Если ЗаписьТребуется Тогда
		ТаблицаСправок2 = ТТНВходящаяОбъект.Товары.Выгрузить(,"Справка2");
		ТаблицаСправок2.Свернуть("Справка2");
		
		МассивСправок2 = ТаблицаСправок2.ВыгрузитьКолонку("Справка2");
		Справки2Поштучная = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивСправок2, "Поштучная");
		
		ВсеСправки2Поштучные = Истина;
		
		Для Каждого КлючИЗначение Из Справки2Поштучная Цикл
			Если КлючИЗначение.Значение <> Истина Тогда
				ВсеСправки2Поштучные = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		ЗаписьТребуется = НЕ ВсеСправки2Поштучные;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(ЗначенияРеквизитов, ТТНВходящаяОбъект);
	
	СвойстваГрузополучателя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТТНВходящаяОбъект.Грузополучатель, "ТорговыйОбъект,Контрагент");
	
	ЗаполнитьЗначенияСвойств(ЗначенияРеквизитов, СвойстваГрузополучателя);
...
Функция ЗапросСтатусаОформленияПередачаВРегистр3ЕГАИСНаОснованииТТНВходящаяЕГАИС()
	
	ТекстЗапроса = ТекстЗапросаСтатусаОформленияДляРегистра2("АктПостановкиНаБалансЕГАИС", "ТТНВходящаяЕГАИС", Истина);
	//Модификация поля Количество входящей ТТН
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"	Товары.Количество",
		"	ВЫБОР
...
Процедура ЗаполнитьТранспортныеРеквизитыТТНИсходящей(ДокументОбъект, ТранспортнаяНакладная = Неопределено)
	...
	Если ЗначениеЗаполнено(ДанныеПеревозки.АвтомобильГосударственныйНомер) Тогда
		ДокументОбъект.Автомобиль = СокрЛП(СтрШаблон("%1 %2", ДанныеПеревозки.АвтомобильМарка, ДанныеПеревозки.АвтомобильГосударственныйНомер));
	КонецЕсли;
