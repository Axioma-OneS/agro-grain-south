...
Процедура ЗаполнитьРегистрыПриВключенииНовойАрхитектуры(Параметры, УникальныйИдентификатор) Экспорт
	...
	Если НЕ ВыполненоОбновлениеРегистровРасчетов Тогда
		Текст = НСтр("ru = 'Включение онлайн взаиморасчетов с партнерами невозможно до конца обновления регистров расчетов.'");
	...
	Если НекорректныеОстатки.Количество() > 0 Тогда
		...
		Для Каждого Организация Из МассивОрганизацийДляОбработки Цикл
			...
			Для Каждого ТипРасчетов Из ТипыРасчетов Цикл
				...
				Движения = КорректировкаОбъект.Движения[ТипРасчетовСтрокой + "ПоСрокам"];
...
Процедура ЗарегистрироватьИзмененияКОтложенномуРаспределению(ЭтоРасчетыСКлиентами, МенеджерВТ, Регистратор) Экспорт
	...
	Для Каждого СтрокаИзменений Из ТаблицаИзменений Цикл
		...
		Если СтрокаИзменений.ПорядокФакт = "" ИЛИ СтрокаИзменений.ПорядокФакт = "30000101" Тогда
			ДатаФакт = Дата(1,1,1);
		Иначе
			ДатаФакт = Дата(Лев(СтрокаИзменений.ПорядокФакт, 8));
		КонецЕсли;
		
		Если СтрокаИзменений.ПорядокПлан = "" ИЛИ СтрокаИзменений.ПорядокПлан = "30000101" Тогда
			ДатаПлан = Дата(1,1,1);
		Иначе
...
Процедура ВыполнитьОтложенноеРаспределение() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыРаспределения.ПорядокФакт = Порядок(Выборка.ДатаПересчета,"",, Тип("ДокументСсылка.РасчетКурсовыхРазниц"));
		ПараметрыРаспределения.ПорядокПлан = Порядок(Выборка.ДатаПересчетаПлан,"",, Тип("ДокументСсылка.РасчетКурсовыхРазниц"));
		
		Если ПараметрыРаспределения.ПорядокФакт = Неопределено Тогда
			ПараметрыРаспределения.ПорядокФакт = "";
		КонецЕсли;
		
		Если ПараметрыРаспределения.ПорядокПлан = Неопределено Тогда
			ПараметрыРаспределения.ПорядокПлан = "";
		КонецЕсли;
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные)
	...
	|			ИЛИ РасчетыПоСрокам.ОбъектРасчетов ЕСТЬ NULL)
	|		//УсловиеПорядок
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		Расчеты.СвязанныйДокумент                                        КАК СвязанныйДокумент,
	|		ВЫБОР 
	|			КОГДА РасчетыПоСрокам.РасчетныйДокумент = Неопределено
	|				ТОГДА РасчетыПоСрокам.ДокументРегистратор
	|			ИНАЧЕ РасчетыПоСрокам.РасчетныйДокумент
	|		КОНЕЦ                                                            КАК РасчетныйДокумент,
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	|				/ РасчетыОтгрузкиПоставки.Сумма КАК ЧИСЛО(31,4))
	|	КОНЕЦ                                                 КАК КурсРегл,
	|	ВЫБОР
	|		КОГДА РасчетыОтгрузкиПоставки.Сумма = 0 
	|			ТОГДА 0
	...
	|				/ РасчетыОтгрузкиПоставки.Сумма КАК ЧИСЛО(31,8))
	|	КОНЕЦ                                                 КАК КурсУпр,
	|	
	...
	Пока КоличествоОбъектовДляПереноса > 0 Цикл
		...
		Если КоличествоСвободныхПотоков > 0 Тогда
			...
			Пока Выборка.Количество() > 0 Цикл
				...
				Поток.НаименованиеЗадания = НСтр("ru = 'Перенос офлайн взаиморасчетов'");
				Поток.ПараметрыПроцедуры.Вставить("ДанныеКОбработке", ДанныеПотока);
				Поток.ПараметрыПроцедуры.Вставить("ВсеДатыПересчета", ВсеДатыПересчета);
...
Функция ВыполнитьОчисткуРегистраМногопоточно(ИмяРегистра) Экспорт
	...
	Потоки = Новый Массив;
	КоличествоСвободныхПотоков = КоличествоПотоковПереходаНаОнлайнВзаиморасчеты();
	КоличествоОбъектовДляОчистки = РазмерПорцииПереносаВОдномПотоке() * КоличествоСвободныхПотоков;
	...
	Пока КоличествоОбъектовДляОчистки > 0 И КонтрольныйЗапуск > 0 Цикл
		...
		Если КоличествоСвободныхПотоков > 0 Тогда
			...
			РазмерВыборки = РазмерПорцииПереносаВОдномПотоке() * КоличествоСвободныхПотоков;
			...
			Если КоличествоОбъектовДляОчистки > 0 Тогда
				...
				Пока ВыборкаДляОчистки.Следующий() Цикл
					...
					Если ДанныеПотока.Количество() = РазмерПорцииПереносаВОдномПотоке() Тогда
...
Процедура УдалитьСлужебныеРегистраторыМногопоточно() Экспорт
	...
	РазмерПорцииОдномПотоке = Цел(Выборка.Количество()/КоличествоПотоковПереходаНаОнлайнВзаиморасчеты()) + 1;
	Потоки = Новый Массив;
	ДанныеПотока = Новый Массив;
	Пока Выборка.Следующий() Цикл
...
Функция КоличествоПотоковПереходаНаОнлайнВзаиморасчеты()
...
Функция КорректировкаРегистров(ПериодКорректировки, Организация, Операция) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Регистр Из ДокументОбъект.ТаблицаРегистров Цикл
			Движения = ДокументОбъект.Движения[Регистр.Имя];
			Движения.Записать();
		КонецЦикла;
...
Функция ТаблицаИзмененийДляПересчета(МенеджерВременныхТаблиц, ЭтоРасчетыСКлиентами, Регистратор) 
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	РасчетыСКлиентамиИзменения.ПорядокОперации                     КАК ПорядокОперации,
	|	РасчетыСКлиентамиИзменения.ПорядокЗачетаПоДатеПлатежа          КАК ПорядокЗачетаПоДатеПлатежа,
	...
	|	РасчетыСКлиентамиИзменения.Сумма                               КАК Сумма
