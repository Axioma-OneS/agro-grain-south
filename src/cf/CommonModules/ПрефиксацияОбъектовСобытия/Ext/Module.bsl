...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	//Карло... 08.12.2020
	//
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаказПоставщику")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаданиеНаПеревозку")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ПоступлениеТоваровНаСклад") Тогда 
		
		ШаблонПрефикса 			= "";
		Префикс 				= "";
		ПрефиксКодаПоставщика 	= "";
		
		Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаказПоставщику") Тогда 
			ПрефиксКодаПоставщика = Источник.НомерПоДаннымПоставщика;
		ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаданиеНаПеревозку") Тогда 
		    ПрефиксКодаПоставщика = Источник.НомерТТН_ЖДН;
		ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ПоступлениеТоваровНаСклад") Тогда
			ПрефиксКодаПоставщика = Источник.НомерВходящегоДокумента;
		КонецЕсли;	
		
		ШаблонПрефикса = "[ОР][ИБ]-[КП]-[Префикс]";
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ОР]", ПрефиксОрганизации);
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ИБ]", ПрефиксИнформационнойБазы);
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[КП]", ПрефиксКодаПоставщика);
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[Префикс]", Префикс);
