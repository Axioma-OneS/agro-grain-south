...
Процедура ЗаполнитьДанныеПоСчету(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента, Отказ) Экспорт
	...
	Если ВидДокумента <> "ДоговорыКонтрагентов" Тогда
		...
		ЭлектронноеВзаимодействиеУТ.ИнициализацияТекстаЗапросаПоискаШтрихкодовПоВременнойТаблицеТоваров(Запрос.Текст, ВидДокумента);
	КонецЕсли;
	...
	Если ВидДокумента <> "ДоговорыКонтрагентов" Тогда
		...
		ШтрихкодыКомбинаций = РезультатЗапроса[7].Выгрузить();
		ШтрихкодыКомбинаций.Индексы.Добавить("Номенклатура, Характеристика, Упаковка");

		ШтрихкодыНоменклатуры = РезультатЗапроса[8].Выгрузить();
		ШтрихкодыНоменклатуры.Индексы.Добавить("Номенклатура");
		
		Если ШтрихкодыКомбинаций.Количество() = 0 ИЛИ ШтрихкодыНоменклатуры.Количество() = 0 Тогда
			ЭлектронноеВзаимодействиеУТ.ШтрихкодыПоТоварам(ТаблицаТоваров, ШтрихкодыКомбинаций, ШтрихкодыНоменклатуры);			
		КонецЕсли;
...
Процедура ЗаполнитьДанныеПоЗаказуТоваров(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента, Отказ) Экспорт
	...
	ВыборкаДляСопоставления = ЭлектронноеВзаимодействиеУТ.ВыборкаДляСопоставленияНоменклатуры(СсылкаНаОбъект);
	ЭлектронноеВзаимодействиеУТ.ИнициализацияТекстаЗапросаПоискаШтрихкодовПоВременнойТаблицеТоваров(Запрос.Текст, Метаданные.Документы.ЗаказПоставщику.Имя, Истина);
	...
	ШтрихкодыКомбинаций = МассивРезультатовЗапроса[6].Выгрузить();
	ШтрихкодыКомбинаций.Индексы.Добавить("Номенклатура, Характеристика, Упаковка");

	ШтрихкодыНоменклатуры = МассивРезультатовЗапроса[7].Выгрузить();
	ШтрихкодыНоменклатуры.Индексы.Добавить("Номенклатура");
...
Процедура ЗаполнитьДанныеПоОтветуНаЗаказ(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента, Отказ) Экспорт
	...
	ВыборкаДляСопоставления = ЭлектронноеВзаимодействиеУТ.ВыборкаДляСопоставленияНоменклатуры(СсылкаНаОбъект);
	ЭлектронноеВзаимодействиеУТ.ИнициализацияТекстаЗапросаПоискаШтрихкодовПоВременнойТаблицеТоваров(Запрос.Текст, Метаданные.Документы.ЗаказКлиента.Имя, Истина);
