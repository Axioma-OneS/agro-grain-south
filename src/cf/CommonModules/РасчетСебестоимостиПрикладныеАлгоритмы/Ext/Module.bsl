...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	#Область Проверки
		...
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "РезультатыПроверок";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Служебный. Хранит произвольные результаты любых проверок.'");
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("ХранилищеЗначения");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Неопределено;
		ОписаниеПараметра.Скрыть			  = Истина; // работа с параметром выполняется программно
		ОписаниеПараметра.Описание =
			НСтр("ru='В данном параметре хранятся произвольные результаты проверок.'");
