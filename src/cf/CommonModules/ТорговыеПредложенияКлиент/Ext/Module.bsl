...
Процедура ОткрытьФормуПодсказок(Форма) Экспорт
	...
	ИначеЕсли Подсказка.Действия.Найти("ПоискПоТоварам") <> Неопределено Тогда
		...
		ОткрытьФормуПоискаПоТоварам(ПараметрыОткрытия, Форма, ОбработчикЗакрытия);
		
	ИначеЕсли Подсказка.Действия.Найти("ПоискПоОтборам") <> Неопределено Тогда
		
		ОткрытьФормуПоискаПоОтборам(Подсказка.ДополнительныеПараметры, Форма, ОбработчикЗакрытия);
...
Процедура ПослеПокупкиКарточкиНоменклатуры(РезультатПокупки, ДополнительныеПараметры) Экспорт 
	...
	ДанныеПокупки = ПолучитьИзВременногоХранилища(РезультатПокупки.АдресРезультата);
...
Процедура ПриЗавершенииСинхронизацииТорговыхПредложений(ДлительнаяОперация, ПараметрыСинхронизации) Экспорт 
	...
	Если Не Отказ Тогда
		...
		Если Не ЭтоАдресВременногоХранилища(ДлительнаяОперация.АдресРезультата) Тогда
			Возврат;
		КонецЕсли;
		
		СтатистикаПубликации = ПолучитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата);
