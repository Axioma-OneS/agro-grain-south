...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Попытка
		...
		ИмяАрхива = ПолучитьИмяФайлаВыгрузки();
...
Функция ПолучитьИмяФайлаВыгрузки()
	...
	Попытка
		...
		Если ЕстьИзмененияВСтруктуреДанных Тогда
			...
			ИмяФайлаВыгрузки = ПолучитьИзВременногоХранилища(АдресДанных);
			
		Иначе			
			МодульВыгрузкаЗагрузкаОбластейДанных = ОбщегоНазначения.ОбщийМодуль("ВыгрузкаЗагрузкаОбластейДанных");
			ИмяФайлаВыгрузки = МодульВыгрузкаЗагрузкаОбластейДанных.ВыгрузитьТекущуюОбластьДанныхВАрхив(АдресДанных);		
		КонецЕсли;
