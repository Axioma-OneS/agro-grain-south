...
Процедура ПередЗаписьюСообщения(ОбъектСообщения, СтандартнаяОбработка) Экспорт
	...
	//Разделенный объект не регистрируется в узле неразделенного плана обмена из неразделенного сеанса
	Если ТипЗнч(ОбъектСообщения) = Тип("СправочникОбъект.СообщенияОбластейДанных") 
		И Не РаботаВМоделиСервиса.ДоступноИспользованиеРазделенныхДанных() Тогда
		
		Для Каждого Получатель Из ОбъектСообщения.ОбменДанными.Получатели Цикл
			ПланыОбмена.ЗарегистрироватьИзменения(Получатель, ОбъектСообщения.Ссылка); 
		КонецЦикла;
