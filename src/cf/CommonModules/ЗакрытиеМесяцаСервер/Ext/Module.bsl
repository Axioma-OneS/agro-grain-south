...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	НоваяСтрока.ТекстПодробнее     = ТекстПодробнееПоУмолчанию();
	НоваяСтрока.ДействиеИспользование = ОписаниеДействия_СервернаяПроцедура(
		"ЗакрытиеМесяцаСервер.Использование_ИсправлениеОшибокВИсходныхДанных");
...
Процедура Использование_ИсправлениеОшибокВИсходныхДанных(ПараметрыОбработчика) Экспорт
	
	Результат = РасчетСебестоимостиКорректировкаСтоимости.ПроверитьНеобходимостьПерепроведенияДокументов_00303344(ПараметрыОбработчика);
	
	Если ЗначениеЗаполнено(Результат.ТекстЗакрытиеМесяца) Тогда 
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			Результат.ТекстЗакрытиеМесяца,
			ТекстПодробнееПоУмолчанию(),
			ОписаниеДействия_ОткрытьФорму(Метаданные.Обработки.ПерепроведениеДокументовПоРегистрамСебестоимости.Формы.Форма.ПолноеИмя()),
