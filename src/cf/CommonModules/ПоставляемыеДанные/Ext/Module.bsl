...
Процедура СохранитьПоставляемыеДанныеВКэш(Знач Дескриптор, Знач ПутьКФайлу) Экспорт
	...
	Если ТипЗнч(Дескриптор) = Тип("Структура") Тогда
		...
		Если ТипЗнч(Дескриптор.Характеристики) = Тип("Массив") Тогда
			Для Каждого Характеристика Из Дескриптор.Характеристики Цикл
				НаАнглийском.Properties.Property.Добавить(Новый Структура("Code, Value, IsKey", Характеристика.Код,
					Характеристика.Значение, Характеристика.Ключевая));
			КонецЦикла;
		КонецЕсли;
	...
	Попытка
		...
		Запрос = ЗапросДанныхПоИменам(Дескриптор.DataType, Фильтр, Ложь);
		...
		КонецЦикла;
		Данные.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе;
...
Процедура УдалитьПоставляемыеДанныеИзКэша(Знач СсылкаИлиИдентификатор) Экспорт
	...
	Если ТипЗнч(СсылкаИлиИдентификатор) = Тип("УникальныйИдентификатор") Тогда
		СсылкаИлиИдентификатор = Справочники.ПоставляемыеДанные.НайтиПоРеквизиту("ИдентификаторФайла",
	...
	Если Не ОбщегоНазначения.СсылкаСуществует(СсылкаИлиИдентификатор) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПоставляемыеДанныеИзКэша(Знач СсылкаИлиИдентификатор) Экспорт
	...
	ДанныеФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаИлиИдентификатор, "ХранимыйФайл").Получить();
	Если ДанныеФайла <> Неопределено Тогда
		Возврат ДанныеФайла;
	КонецЕсли;

	ДанныеФайла = ОбновленныеПоставляемыеДанные(СсылкаИлиИдентификатор);
	Если ДанныеФайла <> Неопределено Тогда
		Возврат ДанныеФайла;
	КонецЕсли;

	УдалитьПоставляемыеДанныеИзКэша(СсылкаИлиИдентификатор);
	Возврат Неопределено;
...
// Возвращаемое значение:
//   Запрос - 
Функция ЗапросДанныхПоИменам(Знач ВидДанных, Знач Характеристики, Знач ТолькоСДанными = Истина)
	Если Характеристики = Неопределено Или Характеристики.Количество() = 0 Тогда
		Возврат ЗапросПоИмениВидаДанных(ВидДанных, ТолькоСДанными);
	Иначе
		Возврат ЗапросПоИменамХарактеристик(ВидДанных, Характеристики, ТолькоСДанными);
	КонецЕсли;
КонецФункции

Функция ЗапросПоИмениВидаДанных(Знач ВидДанных, ТолькоСДанными)
	...
	Если ТолькоСДанными Тогда
		Запрос.Текст = Запрос.Текст + "
									  |	И ПоставляемыеДанные.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВИнформационнойБазе)";
	КонецЕсли;
...
Функция ЗапросПоИменамХарактеристик(Знач ВидДанных, Знач Характеристики, ТолькоСДанными)
// ВЫБРАТЬ Ссылка
// ИЗ Характеристики
...
// ИМЕЮЩИЕ Количество(*) = N.
	...
	|ГДЕ 
	|	ПоставляемыеДанныеХарактеристикиДанных.Ссылка.ВидДанных = &ВидДанных";

	Если ТолькоСДанными Тогда
		Запрос.Текст = Запрос.Текст + "
									  |	И ПоставляемыеДанныеХарактеристикиДанных.Ссылка.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВИнформационнойБазе)";
	КонецЕсли;
	...
	Для Каждого Характеристика Из Характеристики Цикл
		...
		Запрос.Текст = Запрос.Текст + "(
									  | ВЫРАЗИТЬ(ПоставляемыеДанныеХарактеристикиДанных.Значение КАК Строка(150)) = &Значение"
			+ Счетчик + "
...
Функция ИмяСобытияЖР()
	...
	Возврат НСтр("ru = 'Поставляемые данные.Обновление кэша'", ОбщегоНазначения.КодОсновногоЯзыка());

КонецФункции

Функция ОбновленныеПоставляемыеДанные(ЭлементКеша)

	Дескриптор = ДескрипторПоставляемыхДанныхИзКэша(ЭлементКеша);
	ИмяФайлаДанных = РаботаВМоделиСервиса.ПолучитьФайлИзХранилищаМенеджераСервиса(Дескриптор.ИдентификаторФайла);

	Если ИмяФайлаДанных = Неопределено Тогда
		ДескрипторыМС = ДескрипторыПоставляемыхДанныхИзМенеджера(Дескриптор.ВидДанных, Дескриптор.Характеристики);
		Если ДескрипторыМС.Descriptor.Количество() = 0 Тогда
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР(), УровеньЖурналаРегистрации.Ошибка, , , СтрШаблон(НСтр(
			"ru = 'Не удалось получить дескриптор %1'"), ПолучитьОписаниеДанных(Дескриптор)));
			Возврат Неопределено;
		КонецЕсли;
		Дескриптор.ИдентификаторФайла = ДескрипторыМС.Descriptor[0].FileGUID;
		ИмяФайлаДанных = РаботаВМоделиСервиса.ПолучитьФайлИзХранилищаМенеджераСервиса(Дескриптор.ИдентификаторФайла);
		Если ИмяФайлаДанных = Неопределено Тогда
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР(), УровеньЖурналаРегистрации.Ошибка, , , СтрШаблон(НСтр(
			"ru = 'Не удалось получить файл %1 дескриптора %2'"), Дескриптор.ИдентификаторФайла, ПолучитьОписаниеДанных(
			Дескриптор)));
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;

	СохранитьПоставляемыеДанныеВКэш(Дескриптор, ИмяФайлаДанных);

	ДанныеФайла = Новый ДвоичныеДанные(ИмяФайлаДанных);

	ФайлыБТС.УдалитьФайлыВПопытке(ИмяФайлаДанных, ИмяСобытияЖР());
