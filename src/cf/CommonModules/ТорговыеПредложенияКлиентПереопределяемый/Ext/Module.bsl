...
Процедура ОбработатьКомандуФормы(Форма, Команда) Экспорт
	...
	Если СтрНайти(Команда.Имя, "ВставкаИзБуфера") Тогда
		...
		СписокТоваров = ТорговыеПредложенияВызовСервераУТ.ПолучитьСтрокиИзБуфераОбмена();
...
Процедура ОткрытьФормуРедактированияПрайсЛиста(ПозицииТорговыхПредложений, Форма, ОписаниеОповещенияОЗавершении)
	...
	ФормаОбработки = ПолучитьФорму("Обработка.ПрайсЛист.Форма.Форма", , Форма);
	
	ФормаОбработки.ПрименитьНастройки(Неопределено, ДанныеРедактированияПрайсЛиста);
