...
Функция ВыгрузкаНоменклатурыИспользуется(НастройкиПодсистемы = Неопределено) Экспорт
	...
	Возврат НастройкиВыгрузки.ИспользоватьВыгрузкуВКаталог1С = Истина
...
//  Строка - тест запроса.
//
Функция ТекстЗапросаСоответствияДляПубликацииТорговыхПредложений() Экспорт
...
Процедура ПолучитьРеквизитыРубрикатора(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	СопоставленныеРеквизиты.Колонки.Добавить("ФормализованныйРеквизит",         Новый ОписаниеТипов("ПеречислениеСсылка.РеквизитыНоменклатурыДляВыгрузки")); 
	...
	ПодсистемаТорговыеПредложенияСуществует = ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ТорговыеПредложения");
	
	РаботаСНоменклатуройПереопределяемый.ПолучитьДанныеСопоставленияРеквизитов(ПараметрыПроцедуры.Ссылка,
			СопоставленныеРеквизиты, ДополнительныеРеквизиты, РасчетСоответствий);
	Если НЕ СопоставленныеРеквизиты.Количество()
		...
		МодульТорговыеПредложенияПереопределяемый.ПолучитьДанныеСопоставленияРеквизитов(ПараметрыПроцедуры.Ссылка,
			СопоставленныеРеквизиты, ДополнительныеРеквизиты, РасчетСоответствий);
	КонецЕсли;
	...
	Если ПараметрыПроцедуры.Свойство("ПолучитьСопоставленныеФормализованныеРеквизиты")
		...
		Для каждого СопоставленныйРеквизит Из СопоставленныеРеквизиты Цикл
			...
			НовыйРеквизит.ТипЗначения = СопоставленныйРеквизит.РеквизитОбъекта.ТипЗначения;
		КонецЦикла;
	КонецЕсли;
...
Процедура ИзменитьОсновнуюЗаписьСоответствияНоменклатуры(НоменклатураСсылка, ЗначенияЗаполнения)
...
#Область РегистрыСведений_СоответствиеРеквизитовБизнесСеть_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть,"
		+ "РегистрСведений.СоответствиеРеквизитовБизнесСеть";
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.СоответствиеЗначенийРеквизитовБизнесСеть,"
		+ "РегистрСведений.СоответствиеРеквизитовБизнесСеть";
...
#Область РегистрыСведений_СоответствиеНоменклатурыРаботаСНоменклатурой_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление регистра соответствия 1С:Номенклатуры'");
