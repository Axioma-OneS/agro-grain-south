...
Функция ДанныеДляПробитияЧека()
	...
	ДанныеПоТоварам = Документы.ЧекККМКоррекции.ДанныеПоТоварам(Объект, МенеджерВременныхТаблиц);
	ДанныеПоТоварам = Документы.ЧекККМКоррекции.ДанныеКИЗиЕГАИС(Объект, ДанныеПоТоварам);
	
	ДанныеДляИСМП = Документы.ЧекККМКоррекции.ДанныеДляИСМП(Объект, МенеджерВременныхТаблиц);
	
	МассивСтрок = Новый Массив;
	
	ОсобенностиУчета = Новый Массив;
	ОсобенностиУчета.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция);
	ОсобенностиУчета.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция);
	...
	Для каждого СтрокаТЧ Из ДанныеПоТоварам Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
			И ОсобенностиУчета.Найти(СтрокаТЧ.ОсобенностьУчета) <> Неопределено Тогда
			...
			МассивСтрокДляДобавленияВЧек = ДанныеДляИСМП.НайтиСтроки(
				Новый Структура(
					...
					СтрокаТЧ.ИдентификаторСтроки, СтрокаТЧ.НомерСтроки));
		КонецЕсли;
...
Функция РазрешеноИзменятьНоменклатуру(Номенклатура)
	...
	Если ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукция
		...
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция
		...
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.Духи Тогда
