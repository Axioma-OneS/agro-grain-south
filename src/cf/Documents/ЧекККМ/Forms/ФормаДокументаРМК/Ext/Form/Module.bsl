...
Функция ДанныеДляПробитияЧека()
	...
	ДанныеПоТоварам = Документы.ЧекККМ.ДанныеКИЗиЕГАИС(Объект, ДанныеПоТоварам);
	
	ДанныеДляИСМП = Документы.ЧекККМ.ДанныеДляИСМП(Объект, МенеджерВременныхТаблиц);
	
	МассивСтрок = Новый Массив;
	
	ОсобенностиУчета = ИнтеграцияИСКлиентСервер.ВидыПродукцииИСМП(Истина);
	...
	Для каждого СтрокаТЧ из ДанныеПоТоварам Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
			И ОсобенностиУчета.Найти(ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(СтрокаТЧ.ОсобенностьУчета)) <> Неопределено Тогда
		...
		Для каждого СтрокаДляДобавленияВЧек из МассивСтрокДляДобавленияВЧек Цикл
			...
			Если СтрокаПозицииЧека.Количество <> 0 Тогда
				...
				СтрокаПозицииЧека.СуммаСкидок    = СтрокаПозицииЧека.Количество * СтрокаТЧ.Цена - СтрокаДляДобавленияВЧек.Сумма;
			КонецЕсли;
