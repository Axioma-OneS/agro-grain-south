...
Функция ДанныеДляИСМП(Объект, МенеджерВременныхТаблиц) Экспорт
	...
	|	ТоварыПоСериям.ОсобенностьУчета,
	...
	КонецЕсли;
	ШтрихкодыУпаковок = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(МассивУпаковок, ПараметрыСканирования, Истина);
...
Функция ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если Товары.Количество() > 0 Тогда
		...
		ВсегоВКеше = ПроверкаИПодборПродукцииИС.КоличествоПоДаннымКеша(Форма, ПоляПоиска);
