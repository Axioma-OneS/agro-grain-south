...
Функция СформироватьПечатнуюФормуАктСверкиВзаиморасчетов(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ДанныеДокумента Из ДанныеСверки.Строки Цикл
		...
		Если ДанныеДокумента.НачалоПериода = '00010101' Тогда
			...
			ПредставлениеПериода = СтрШаблон(НСтр("ru = 'по состоянию на %1'"), Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД"),);
			
			ПредставлениеПериодаРасчетов = СтрШаблон(НСтр("ru = 'По состоянию на %1'"), Формат(ДанныеДокумента.КонецПериода, "ДЛФ=ДД"),);
		...
		Иначе
			...
			Если  КодСимвола(ПервыйСимволПредставлениеПериода) >= 48
				...
				ПредставлениеПериода = СтрШаблон(НСтр("ru='за период: %1'"), ПредставлениеПериода);
				
			Иначе
				ПредставлениеПериода = СтрШаблон(НСтр("ru='за %1'"), ПредставлениеПериода);
		...
		Если ЗначениеЗаполнено(ДанныеДокумента.Договор) Тогда
			ОбластьМакета.Параметры.ПредставлениеДоговора = НСтр("ru = 'по договору'") + ": " + ДанныеДокумента.ПредставлениеДоговора;
		КонецЕсли;
...
Функция ВывестиДетальныеЗаписи(
	...
	Валюта = СтрШаблон(НСтр("ru='в валюте'") + " %1", ДанныеПоОбъектуГруппировки.Валюта);
	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов = "ОбъектРасчетов"
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru = 'по объекту расчетов'") + " %1 ";
	ИначеЕсли НастройкиПечати.ДетализацияВзаиморасчетов = "Договор" И НЕ ЭтоФинансовыеИнструменты
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru = 'по договору'") + " %1 ";
	КонецЕсли;
