...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Карло... 27.08.2020
	Элементы.ЗаданиеВыполняет.Видимость 		= Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	// Карло... 04.10.2020
	Элементы.СтатусПодтверждения.Доступность = ПроверкаРоли("БухгалтерПоПервичнойДокументации") Или ПроверкаРоли("ПолныеПрава"); 
	Если Объект.СтатусПодтверждения = ПредопределенноеЗначение("Перечисление.СтатусыПодтверждения.ПодтвержденоБухгалтерией") Тогда 
		ЭтаФорма.ТолькоПросмотр = Не ПроверкаРоли("БухгалтерПоПервичнойДокументации");
	КонецЕсли;
	// ...И.В.
...
// Карло... 07.12.2020
&НаКлиенте
Процедура ЦенаПеревозкиНаТонну_с_НДСПриИзменении(Элемент)
	ПересчитатьЦенуПеревозкиНаТоннуБезНДС();
КонецПроцедуры
...
Процедура ДобавитьРаспоряжение(Команда)
	...
	Если Объект.Маршрут.Количество() > 0 Тогда 
		ПоказатьПредупреждение(,НСтр("ru = 'На вкладке Маршрут более одной строки. Невозможно записать документ.'"));
		Возврат;
	КонецЕсли;
	//
	// ...И.В.
...
&НаКлиенте
Процедура СоздатьНомерТранспорта(Команда)
	СоздатьНомерТранспортаНаСервере();
КонецПроцедуры

// Карло... 27.08.2020
&НаСервере
Процедура СоздатьНомерТранспортаНаСервере()
	
	НайденныйОбъект = Справочники.ТранспортныеСредства.НайтиПоНаименованию(Объект.НомерВагона, Истина);
	Если НайденныйОбъект = Справочники.ТранспортныеСредства.ПустаяСсылка() Тогда 
		
		ГрузоподъемностьВТоннах = Объект.Маршрут.Итог("Вес");
		
		НовыйЭлемент = Справочники.ТранспортныеСредства.СоздатьЭлемент();
		НовыйЭлемент.Код 						= Объект.НомерВагона;
		НовыйЭлемент.Наименование 				= Объект.НомерВагона;
		НовыйЭлемент.ЛицензионнаяКарточкаВид 	= "Стандартная";
		НовыйЭлемент.ВместимостьПредставление   = Строка(ГрузоподъемностьВТоннах);
		НовыйЭлемент.ГрузоподъемностьВТоннах    = ГрузоподъемностьВТоннах;
		НовыйЭлемент.Записать();
		
		Объект.ТранспортноеСредство = НовыйЭлемент.Ссылка;
	Иначе
	    Объект.ТранспортноеСредство = НайденныйОбъект;
	КонецЕсли;	
...
// Карло... 07.12.2020
&НаКлиенте
Процедура ПересчитатьЦенуПеревозкиНаТоннуБезНДС()
	
	Если Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС") Тогда 
		ПроцентНДС = 0;
	ИначеЕсли Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС0") Тогда 	
		ПроцентНДС = 0;
	ИначеЕсли Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС10") Тогда 	
		ПроцентНДС = 10;
	ИначеЕсли Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС18") Тогда 	
		ПроцентНДС = 18;
	ИначеЕсли Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20") Тогда 	
		ПроцентНДС = 20;
	Иначе 
		ПроцентНДС = 0;
	КонецЕсли;
	Объект.ЦенаПеревозкиНаТонну_без_НДС = Объект.ЦенаПеревозкиНаТонну_с_НДС / (ПроцентНДС + 100) * 100;
