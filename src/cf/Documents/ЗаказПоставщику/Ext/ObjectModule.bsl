...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Модифицированность()
		...
		Отказ = Истина;
		ТекстСообщения = "На вкладке Товары более одной строки. Невозможно записать документ.";
	...
	Если (ЭтотОбъект.ПометкаУдаления) И (НЕ Ссылка.ПометкаУдаления) Тогда
		...
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |ГДЕ
		                      |	ЗаявкаНаУслуги.Проведен
		                      |	И ЗаявкаНаУслуги.Распоряжение = &Распоряжение");
		Запрос.УстановитьПараметр("Распоряжение", Ссылка);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда 
			Отказ = Истина;
			ТекстСообщения = СтрШаблон("На основании документа %1 созданы перевозки и доп. услуги.", Ссылка);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			
		ИначеЕсли Не РольДоступна("КоммерческийДиректор") Тогда 
			...
			Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
			                      ...
			                      |ГДЕ
			                      |	ИсполнителиЗадач.РольИсполнителя = ЗНАЧЕНИЕ(Справочник.РолиИсполнителей.СогласующийКоммерческиеУсловияЗакупок)");
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда 
			...
			ЗадачаОбъект.ДатаНачала						= ТекущаяДатаСеанса();
			...
			ЗадачаОбъект.РольИсполнителя 				= Справочники.РолиИсполнителей.СогласующийКоммерческиеУсловияЗакупок;
			ЗадачаОбъект.Описание 						= НСтр("ru = 'Удалить документ.'");
			ЗадачаОбъект.Наименование 					= НСтр("ru = 'Удалить документ.'");
			ЗадачаОбъект.Предмет						= Ссылка;
			ЗадачаОбъект.Записать();
						
			ТекстСообщения = СтрШаблон("Требуется согласовать удаление документа %1 с комменрческим директором, задача на удаление отправлена ответственному.", Ссылка);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
