...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.СтраницаДоставкаНашимиСилами.Видимость 		= Ложь;
	Элементы.ГруппаСводнаяИнформацияПоДоставке.Видимость 	= Ложь;
	Если Объект.СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПоставщикаДоНашегоСклада") Тогда 
		СпособДоставки = "Поставщик (на наш склад)";	
	ИначеЕсли Объект.СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчикаДоНашегоСклада") Тогда
		СпособДоставки = "Перевозчик (на наш склад)";
	Иначе 
...
// Карло... 22.10.2020
&НаКлиенте
Процедура СпособДоставки1ПриИзменении(Элемент)
	
	Если СпособДоставки = "Поставщик (на наш склад)" Тогда 
		Объект.СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПоставщикаДоНашегоСклада");		
	ИначеЕсли СпособДоставки = "Перевозчик (на наш склад)" Тогда 	
		Объект.СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчикаДоНашегоСклада");
	Иначе 
...
// Карло... 29.12.2020
&НаКлиенте
Процедура ПоступленияНаСкладЗавершеныПриИзменении(Элемент)
	Если Объект.ПоступленияНаСкладЗавершены Тогда 
		Объект.ПоступленияНаСкладЗавершеныДата = ТекущаяДата();
	Иначе 
	    Объект.ПоступленияНаСкладЗавершеныДата = Дата(1,1,1);
	КонецЕсли;	
КонецПроцедуры
