...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТовара Из ЭтотОбъект.Товары Цикл 	
		Если Не СтрокаТовара.ДоставкаПоставщика И СтрокаТовара.Аналитика.Пустая() Тогда 
			Сообщить("На вкладке Товары более одной строки. Невозможно записать документ");
			Отказ = Истина;
		КонецЕсли;		
	КонецЦикла;	
	// ...И.В.
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Для Каждого строкаТовар из Товары Цикл 
		...
		строкаТовар.КоличествоУпаковок 	= 0;		
	КонецЦикла;
	//
	// ...И.В.
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Модифицированность()
		И Товары.Количество() > 1 Тогда 
		Отказ = Истина;
		ТекстСообщения = "Не возможно записать документ. В заказе больше одной строки с товарами!";
	...
	Если Не ЭтоНовый()
		...
		И ЭтотОбъект.Ссылка.СтатусПодтверждения = Перечисления.СтатусыПодтверждения.ПодтвержденоБухгалтерией Тогда 		
		
		СтатусПодтверждения = Перечисления.СтатусыПодтверждения.НеСоответствуетДаннымБухгалтерии;
		СтатусПодтвержденияДата = Дата(1,1,1);
		Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл  
			Если Не ПользовательИБ.Роли.Содержит(Метаданные.Роли.БухгалтерПоПервичнойДокументации) Тогда 
				Продолжить;
			КонецЕсли;	
			ЗадачаОбъект = Задачи.ЗадачаИсполнителя.СоздатьЗадачу();
			ЗадачаОбъект.Наименование 	= СтрШаблон("Повторная сверка с оперативными данными, %1", Ссылка);
			ЗадачаОбъект.Важность 		= Перечисления.ВариантыВажностиЗадачи.Обычная;
			ЗадачаОбъект.СрокИсполнения = КонецДня(ТекущаяДата());
			ЗадачаОбъект.Исполнитель 	= Справочники.Пользователи.НайтиПоРеквизиту("ИдентификаторПользователяИБ", ПользовательИБ.УникальныйИдентификатор);
			ЗадачаОбъект.Предмет 		= Ссылка;
			ЗадачаОбъект.Автор 			= Пользователи.ТекущийПользователь();
			ЗадачаОбъект.Дата 			= ТекущаяДатаСеанса();
			ЗадачаОбъект.Описание 		= СтрШаблон("Требуется сверить документ с оперативными данными, после перепроведения пользователем %1", Пользователи.ТекущийПользователь());
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого СтрокаТовары Из Товары Цикл 
		Если СтрокаТовары.Аналитика <> Документы.ЗаданиеНаПеревозку.ПустаяСсылка() Тогда 
			ОбЗаданиеНаПеревозку = СтрокаТовары.Аналитика.ПолучитьОбъект();
			ОбЗаданиеНаПеревозку.Статус = Перечисления.СтатусыЗаданийНаПеревозку.Закрыто;
			Попытка
				ОбЗаданиеНаПеревозку.Записать(РежимЗаписиДокумента.Проведение);	
			Исключение	
				ОбЗаданиеНаПеревозку.Записать(РежимЗаписиДокумента.Запись);
				ТекстСообщения = СтрШаблон("Не удалось изменить статус у документа : %1", ОбЗаданиеНаПеревозку.Ссылка);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
				ЗаписьЖурналаРегистрации(".Документ.ПоступлениеТоваровНаСклад.ОбработкаПроведения",УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
			КонецПопытки;	
		КонецЕсли;
	КонецЦикла;
	//
