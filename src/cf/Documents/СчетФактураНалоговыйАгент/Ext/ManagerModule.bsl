...
Процедура ИнициализироватьВтДокументыПоступления(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		|	ГДЕ
		|		ТИПЗНАЧЕНИЯ(Расчеты.ДокументРегистратор) В (&ТипыДокументовПриобретения)
		...
		|		И ТИПЗНАЧЕНИЯ(Расчеты.ОбъектРасчетов) В (&ТипыДокументовПриобретения)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|		И ТИПЗНАЧЕНИЯ(Расчеты.РасчетныйДокумент) В (&ТипыДокументовПриобретения)) КАК ДокументыПоступления
		|
		|СГРУППИРОВАТЬ ПО
	...
	Иначе
		...
		|			ПО Расчеты.АналитикаУчетаПоПартнерам = АналитикаУчетаПартнеров.Ссылка
		|	ГДЕ
		|		ТИПЗНАЧЕНИЯ(Расчеты.Регистратор) В (&ТипыДокументовПриобретения)
		...
		|		И Расчеты.РасчетныйДокумент = &ДокументОснование
		|		И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|		И ТИПЗНАЧЕНИЯ(Расчеты.ЗаказПоставщику) В (&ТипыДокументовПриобретения)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
