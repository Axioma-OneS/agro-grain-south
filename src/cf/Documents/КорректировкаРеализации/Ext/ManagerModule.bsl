...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	|	ТабличнаяЧасть.Содержание КАК Содержание,
	|	ВЫБОР
	...
	|ПОМЕСТИТЬ ТоварыДокументаСГруппировкой
	|ИЗ
	...
	|	ТабличнаяЧасть.Номенклатура КАК Номенклатура,
	|	ТабличнаяЧасть.Характеристика КАК Характеристика,
	|	ТабличнаяЧасть.КодВидаНоменклатурнойКлассификации,
	|	ТабличнаяЧасть.Серия КАК Серия,
	|	ТабличнаяЧасть.Упаковка КАК Упаковка,
	|	ТабличнаяЧасть.СтатусУказанияСерий КАК СтатусУказанияСерий,
	|	ЕСТЬNULL(ТабличнаяЧасть.Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) КАК
	|		ТипНоменклатуры,
	...
	|ИЗ
	|	ТоварыДокументаСГруппировкой КАК ТабличнаяЧасть
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТаблицаТоваровСРаспределениемПоВидамЗапасов(Товары, ДанныеДляВидовЗапасов(ДокументСсылка, Шапка.ДокументОснование, Шапка.ТребуетсяЗалогЗаТару));
	ПодключаемоеОборудованиеУТВызовСервера.ТаблицаТоваровСРаспределениемПоИСМП(Товары, ДанныеДляИСМП(ДокументСсылка, МенеджерВременныхТаблиц));
...
Функция ДанныеДляИСМП(ДокументСсылка, МенеджерВременныхТаблиц)
	...
	Если ПараметрыСканирования.Свойство("ОтборПоВидуПродукции") Тогда
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Склад,Дата");
	ПараметрыУказанияСерий = ПараметрыУказанияСерий(РеквизитыДокумента);
	
	Возврат ПодключаемоеОборудованиеУТВызовСервера.ТаблицаМаркированныхТоваровСоШтрихкодамиУпаковок(
		Товары, ШтрихкодыУпаковок.МаркированныеТовары, ПараметрыУказанияСерий);
	
КонецФункции

Функция ДанныеДляВидовЗапасов(ДокументСсылка, ДокументОснование, ТребуетсяЗалогЗаТару)
