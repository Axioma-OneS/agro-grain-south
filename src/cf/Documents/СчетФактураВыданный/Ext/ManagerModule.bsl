...
Функция ТекстЗапросаВтТаблицаПодтвержденияСтавки0(Запрос, ТекстыЗапроса)
	...
	|		ИЛИ &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	|";
...
#Область ТекстКорректировка
	...
	|	КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	...
	|	ПО ИСТИНА
	|ГДЕ
	|	ТаблицаДанных.Сумма > 0
	|	И НЕ ТаблицаДанных.Комиссия
	|	И (&Корректировочный ИЛИ &Исправление)
	|	И &Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	И (НЕ &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
...
Функция ТекстЗапросаДанныхМаркировки()
	...
	|	СчетаФактурыОснования.Ссылка КАК Ссылка
	|ИЗ ШтрихкодыУпаковок КАК ТаблицаШтрихкодыУпаковок
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаФактурыОснования КАК СчетаФактурыОснования
