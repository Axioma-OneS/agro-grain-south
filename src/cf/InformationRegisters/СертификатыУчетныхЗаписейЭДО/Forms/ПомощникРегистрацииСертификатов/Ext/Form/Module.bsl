...
Функция ДанныеПоРегистрационнойИнформацииНаСервере(СтруктураДанных, КонтекстОперации)
	...
	ПодключенПоOnboarding = ОбменСКонтрагентамиСлужебный.БазаСозданаЧерезOnboarding();
	Если ПодключенПоOnboarding Тогда
		РеквизитыПакета.Вставить("ПодключенПоOnboarding",    ПодключенПоOnboarding);
	КонецЕсли;
