...
Процедура ЗаполнитьИдентификаторыОрганизации(СписокИдентификаторов = Неопределено)
	...
	Если СписокИдентификаторов = Неопределено Тогда
		СписокИдентификаторов = ОбменСКонтрагентамиСлужебный.СписокИдентификаторовУчетныхЗаписейОрганизации(Организация);
	КонецЕсли;
...
Процедура НастроитьОсновнуюСтраницуФормы(Форма)
	...
	Этап3Завершен = Этап2Завершен
		...
		И (ЗначениеЗаполнено(Форма.ИмяФайлаСоглашенияНаРоуминг)
			...
			Или Форма.ВариантПотребностиСоглашения = ПредопределенноеЗначение("Перечисление.ВариантыПотребностиСоглашенияНаРоуминг.НеОбязательно"))
		И ЭлектроннаяПочтаКорректна
		И (ЭлектроннаяПочтаОрганизацииКорректна Или Не Форма.РоумингБезАвтоматическойНастройки Или Форма.РежимНастройки);
	...
	Иначе
		...
		Если Форма.ТребуетсяСоглашение
...
Функция ТребуетсяЗаполнитьEmailОрганизации(Форма) 
	...
	Возврат Форма.РоумингБезАвтоматическойНастройки И Не Форма.РежимНастройки;
