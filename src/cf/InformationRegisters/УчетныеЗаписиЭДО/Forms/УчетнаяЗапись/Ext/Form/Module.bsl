...
Процедура ПослеПолученияОтпечатков(Отпечатки, Контекст) Экспорт
	...
	Если ТипЗнч(Отпечатки) = Тип("Строка") Тогда
		...
		Сертификаты = ВсеСертификатыИСамыйНовый(Контекст.СписокСертификатов, Неопределено);
	Иначе 
...
&НаСервереБезКонтекста
Функция ВсеСертификатыИСамыйНовый(Знач СписокСертификатов, Отпечатки = Неопределено)
