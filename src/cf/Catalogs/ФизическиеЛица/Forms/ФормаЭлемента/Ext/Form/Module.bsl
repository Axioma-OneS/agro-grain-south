...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если МобильныйТелефонНеУказан_НаСервере() Тогда 
		ПоказатьПредупреждение(, "Рекомендуется указать мобильный телефон", 20, "Запись физического лица");
	КонецЕсли;
КонецПроцедуры

// Карло... 01.11.2020
&НаСервере
Функция МобильныйТелефонНеУказан_НаСервере()
