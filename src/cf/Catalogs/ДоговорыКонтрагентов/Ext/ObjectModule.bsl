...
Процедура ПередЗаписью(Отказ)
	...
	// Карло... 07.04.2021
	// Описание: Переносим значение реквизитов: "Поставщик", "Перевозчик", "ДопУслуги" и "Хранение"
	//				в партнера договора
	//
	...
	Если Перевозчик Тогда
		ОбъектПартнер.Перевозчик = Истина;					
	КонецЕсли;
	Если ДопУслуги Тогда
		ОбъектПартнер.ДопУслуги = Истина;					
	КонецЕсли;
	Если Хранение Тогда
		ОбъектПартнер.Хранение = Истина;					
	КонецЕсли;
	Попытка
		ОбъектПартнер.Записать();
	Исключение
	    ТекстСообщения = СтрШаблон("Не возможно записать партнера, по причине: %1", ОписаниеОшибки());
		ЗаписьЖурналаРегистрации("", УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Партнеры, Партнер, ТекстСообщения);
	КонецПопытки;
	//
	// ... И.В.
