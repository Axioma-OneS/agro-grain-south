...
Процедура НастроитьФорму()
	...
	РазрешеноИспользоватьСерии = (ЭтоТовар Или ЭтоМногооборотнаяТара)
									...
									И ((Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
												 И (ИспользоватьОрдерныеСклады
												 	И ИспользоватьНесколькоСкладов
													Или ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач)
										Или Не (Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция));
	...
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСОдинСкладБезОрдерности.Видимость = (Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
																			И Не ИспользоватьНесколькоСкладов
																			И Не ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач;
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСНетОрдерныхСкладов.Видимость = (Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
			 																И ИспользоватьНесколькоСкладов
																			И Не ИспользоватьОрдерныеСклады;
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСЕстьОрдерныеСклады.Видимость = (Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция)
	...
	Элементы.НадписьПодсказкаПолитикиУчетаОтключеныОрдерныеСклады.Видимость = РазрешеноИспользоватьСерии
																	...
																	И Не Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция;
...
Процедура УстановитьНастройкуИспользованияСерий()
	...
	Если Объект.ИспользоватьСерии Тогда
		...
		Если Объект.ПодконтрольнаяПродукцияВЕТИС Или Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция Тогда
			Объект.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваров;
		Иначе
...
Функция СовместимыеОсобенностиУчета(ТекущаяОсобенностьУчета, ЗначащаяЧастьИмениНадписи)
	...
	ИначеЕсли ЗначащаяЧастьИмениНадписи = "Серий" Тогда
		...
		ИначеЕсли ТекущаяОсобенностьУчета = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры.МолочнаяПродукция") Тогда	
...
Процедура ЗаписатьСвойстваДопРеквизитов(НазначениеДопРеквзитов, Отказ)
	...
	Пока ВыборкаПоСвойствам.Следующий() Цикл
		...
		СвойствоОбъект = ВыборкаПоСвойствам.Свойство.ПолучитьОбъект();
