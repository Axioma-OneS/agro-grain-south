...
Процедура СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку)
	...
	Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
		...
		СтрокаСФ.СуммаНДС = Окр(Сумма * 20 / 120);
...
Функция ПреобразоватьСсылкуВПервичныйДокумент(ДокументСсылка, ДокументДата, ДокументНомер, ТипПервичногоДокумента = Неопределено, Сумма = 0, СуммаРегл = 0, ТолькоПоиск = Ложь)
	...
	Если ТипПервичногоДокумента = Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СтруктураРеквизитовШапки = Новый Структура("Организация, ОрганизацияПолучатель, Контрагент, Партнер, Договор, Валюта, ДатаВходящегоДокумента, НомерВходящегоДокумента, СуммаДокумента, СуммаВзаиморасчетов, ПорядокРасчетов");
	...
	Если ЗначениеЗаполнено(ОбъектРасчетов) И 
		...
		Если СтруктураРеквизитовШапки.Свойство("Дата") И СтруктураРеквизитовШапки.Свойство("Номер") Тогда
			...
			Если ЗначениеЗаполнено(ПДСсылка) Тогда
				// Подставим в объект расчетов созданный первичный документ.
