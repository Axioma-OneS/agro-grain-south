		...
		Если ИнтернетПоддержкаПодключена Тогда
			Если ОперацияЗагрузкаКатегорий <> Неопределено Тогда
				Если ЗначениеЗаполнено(ТекущаяКатегория) Тогда
					ЗагрузитьКатегорииСервиса(ТекущаяКатегория, РежимВыбораТорговогоПредложения);
				Иначе
					ЗагрузитьКатегорииСервиса();
				КонецЕсли;
			КонецЕсли;
		Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ТорговыеПредложения_ПоискПоОтборам_Обновить" Тогда
		...
		Если ПараметрыИзменены Тогда
			Если ЕстьПодсистемаРаботаСНоменклатурой Тогда
				ЗагрузитьКатегорииСервиса(ИдентификаторКатегории, ИдентификаторКатегории <> Неопределено);
			КонецЕсли;
...
Процедура ПоказатьСкрытьКатегории(Команда)
	...
	Если Элементы.ПоказатьСкрытьКатегории.Пометка
		...
		ЗагрузитьКатегорииСервиса(ТекущаяКатегория, Истина);
	КонецЕсли;
...
Процедура КатегорииПередРазворачиванием(Элемент, Строка, Отказ)
	...
	ТекущаяКатегория       = СтрокаДерева.Идентификатор;
...
Процедура ДеревоОтборовПриИзменении(Элемент)
	...
	Если (ТекущаяСтрока.ФиксированныйОтбор И НЕ ТекущаяСтрока.ФиксированноеЗначение
		...
		ИЛИ ТекущаяСтрока.Идентификатор = "Поставщик" Тогда
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьКартинкуКомандыНайти(ЭтоДлительнаяОперация, НайтиПредложениеКартинка)
	
	Если ЭтоДлительнаяОперация Тогда
		НайтиПредложениеКартинка.Картинка = БиблиотекаКартинок.ДлительнаяОперация16;
	Иначе
		НайтиПредложениеКартинка.Картинка = БиблиотекаКартинок.УправлениеПоиском;
	КонецЕсли;
...
//
&НаСервере
Функция НайтиТорговыеПредложенияВФоне(ЗаполненныеПараметрыЗапроса = Неопределено, Знач СледующаяСтраница = Ложь)
	...
	Если НЕ Отказ И Результат.Статус = "Выполнено" Тогда
		Если ЗначениеЗаполнено(Результат.АдресРезультата)
			И ЭтоАдресВременногоХранилища(Результат.АдресРезультата) Тогда
				...
				ДанныеПоискаИзСервиса = ОбработатьРезультатПоиска(Результат.АдресРезультата);
				ЗаполнитьТаблицуТорговыхПредложений(ДанныеПоискаИзСервиса);
...
&НаКлиенте
Процедура ЗаполнитьТаблицуТорговыхПредложений(ДанныеПоискаИзСервиса)
...
Процедура ДобавитьТоварыДляЗаказа(ПараметрыТовара, Количество)
	...
	РассчитатьСуммыСтрокиТоваров(СтрокаТоваров, Количество);
	
КонецПроцедуры

&НаСервере
Процедура РассчитатьСуммыСтрокиТоваров(СтрокаТоваров, Количество)
...
Процедура УстановитьВидимостьДоступность()
	...
	НастроитьЭлементыРаботаСНоменклатурой(Ложь);
...
&НаКлиенте
Процедура ЗагрузитьКатегорииСервиса(ИдентификаторКатегории = Неопределено, ВосстановитьДеревоКатегорий = Ложь)
	...
	Если НЕ ТребуетсяЗагрузка Тогда
		Если РежимВыбораТорговогоПредложения
			И ТекущаяВетка = Элементы.ДеревоОтборов.ТекущиеДанные
			ИЛИ НЕ РежимВыбораТорговогоПредложения Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ПолучитьДанныеИзКэша = Ложь;
	ДлительнаяОперация = ПолучитьОперациюДляЗапросаВСервисе(ИдентификаторКатегории, ВосстановитьДеревоКатегорий, ПолучитьДанныеИзКэша);
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		...
		ПолучитьДанныеСервисаЗавершение = Новый ОписаниеОповещения("ЗагрузитьКатегорииСервисаЗавершение",
	...
	Если ПолучитьДанныеИзКэша Тогда
		...
		ЗагрузитьКатегорииСервисаЗавершение(Неопределено, ДополнительныеПараметры);
	Иначе
		МодульРаботаСНоменклатуройСлужебныйКлиент.УстановитьВидимостьДоступностьЭлементовКатегорийПриДлительнойОперации(
			Элементы, КоличествоЗаданий);
	КонецЕсли;
...
Функция ПолучитьОперациюДляЗапросаВСервисе(ИдентификаторКатегории, ВосстановитьДеревоКатегорий,
			ПолучитьДанныеИзКэша)
			
	МодульРаботаСНоменклатуройСлужебныйКлиент       = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройСлужебныйКлиент");
	МодульРаботаСНоменклатуройСлужебныйВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройСлужебныйВызовСервера");
	КэшКатегорий = МодульРаботаСНоменклатуройСлужебныйКлиент.ПолучитьКэшКатегорий();
	
	Если ИдентификаторКатегории = Неопределено И НЕ ЗначениеЗаполнено(СтрокаПоискаКатегорий) Тогда
	...
	ИначеЕсли ВосстановитьДеревоКатегорий Тогда
		...
		КоличествоЗаданийПоискаТП = КоличествоЗаданийПоискаТП + 1;
...
Процедура ЗагрузитьКатегорииСервисаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТекущиеДанные <> Неопределено
		И ТекущиеДанные.Наименование <> ПредставлениеКатегории Тогда
		ПредставлениеКатегории = ТекущиеДанные.Наименование;
		ЗаполнитьЗаголовокФормы();
	КонецЕсли;
