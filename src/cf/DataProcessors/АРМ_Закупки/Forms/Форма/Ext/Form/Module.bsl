...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.СтраницыДокументы.Видимость Тогда 
		Если Элементы.СтраницыДокументы.ТекущаяСтраница = Элементы.СтраницаСпецификация Тогда
...
Функция СохранитьНаСервере()
	...
	ИначеЕсли СтрДокументы.Видимость И СтрПрочее.Видимость Тогда  
		Если СтрДокументы.ТекущаяСтраница = Элементы.СтраницаСпецификация
			...
			УспешноСпецификация = СохранитьДокумент_ЗаказПоставщика(ПараметрыЗаполнения);
...
Функция СохранитьСправочник_Договор(Параметры)
	...
	ОбъектДоговор.ВалютаВзаиморасчетов 	= Параметры.Валюта;
	ОбъектДоговор.Организация 			= Параметры.Организация;
	
	ОбъектДоговор.Наименование          = ТекстНаименованияДоговора;
	ОбъектДоговор.НаименованиеДляПечати = ТекстНаименованияДоговора;
	ОбъектДоговор.Контрагент 			= НайденныйКонтрагент;
	ОбъектДоговор.Партнер 				= Договор_Партнер;
	ОбъектДоговор.Поставщик 			= Договор_Поставщик;
	ОбъектДоговор.Перевозчик 			= Договор_Перевозчик;
	ОбъектДоговор.ДопУслуги 			= Договор_ДопУслуги;
	ОбъектДоговор.Хранение 				= Договор_Хранение;
	ОбъектДоговор.Номер 				= Договор_Номер;
	ОбъектДоговор.Дата 					= Договор_ДатаЗаключения;
	ОбъектДоговор.ДатаНачалаДействия 	= Договор_ДействуетС;
	ОбъектДоговор.ДатаОкончанияДействия = Договор_ДействуетПо;
				
	ОбъектДоговор.Менеджер 										= Пользователи.ТекущийПользователь();	
	ОбъектДоговор.ПорядокОплаты 								= Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВРублях;	
	ОбъектДоговор.ПорядокРасчетов 								= Перечисления.ПорядокРасчетов.ПоНакладным;		
	ОбъектДоговор.Статус 										= Перечисления.СтатусыДоговоровКонтрагентов.НеСогласован;
	ОбъектДоговор.ХозяйственнаяОперация 						= Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика;	
	...
	ОбъектДоговор.НалогообложениеНДС 							= Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;	
	ОбъектДоговор.ЗакупкаПодДеятельность 						= Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
	ОбъектДоговор.НалогообложениеНДСОпределяетсяВДокументе 		= Истина;
	ОбъектДоговор.ЗакупкаПодДеятельностьОпределяетсяВДокументе 	= Истина;
	ОбъектДоговор.ВариантПриемкиТоваров 						= Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным;
	ОбъектДоговор.СпособДоставки 								= Перечисления.СпособыДоставки.ОпределяетсяВРаспоряжении;
	
	ОбъектДоговор.Комментарий = Комментарий;
	...
	Попытка
		...
		СсылкаНаОбъект 	= ОбъектДоговор.Ссылка;
		Успешно 		= Истина;
