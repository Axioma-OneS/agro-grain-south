...
Процедура ОснМеню_ТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	ИначеЕсли Элемент.ТекущаяОбласть.Текст = "Добавить поступление CPT" Тогда 			
		ПараметрыЗаполнения = Новый Структура("Расшифровка, ОбъемПоступления, ДатаПоступления, Склад");
		ПараметрыЗаполнения.Вставить("Сделка"			, Расшифровка);
		ПараметрыЗаполнения.Вставить("ОбъемПоступления"	, ОснМеню_ТабличныйДокумент.Область(Элемент.ТекущаяОбласть.Верх, 7, Элемент.ТекущаяОбласть.Низ, 7) );
		ПараметрыЗаполнения.Вставить("ДатаПоступления"	, ОснМеню_ТабличныйДокумент.Область(Элемент.ТекущаяОбласть.Верх, 8, Элемент.ТекущаяОбласть.Низ, 8) );
		ПараметрыЗаполнения.Вставить("Склад"			, ОснМеню_ТабличныйДокумент.Область(Элемент.ТекущаяОбласть.Верх, 9, Элемент.ТекущаяОбласть.Низ, 9) );
...
Процедура Перевозка_ЗаполнитьПоступлениеНаСкладПриИзменении(Элемент)
	...
	Элементы.Перевозка_ОбъемПоступления.АвтоОтметкаНезаполненного 	= Перевозка_ЗаполнитьПоступлениеНаСклад;
	Элементы.Перевозка_ПотериНаПеревозке.АвтоОтметкаНезаполненного 	= Перевозка_ЗаполнитьПоступлениеНаСклад;
	...
	Если Перевозка_БазисПоставки = "EXW"
		...
		Элементы.Перевозка_АдресПоступления_Город.АвтоОтметкаНезаполненного 			= Ложь;
		Элементы.Перевозка_АдресПоступления_СкладПоступления.АвтоОтметкаНезаполненного 	= Ложь;
		Элементы.Перевозка_ДатаПоступления.АвтоОтметкаНезаполненного 					= Ложь;
	Иначе 
		Элементы.ГруппаАдресПоступления.Видимость = Истина;
		Элементы.Перевозка_АдресПоступления_Город.АвтоОтметкаНезаполненного 			= Истина;
		Элементы.Перевозка_АдресПоступления_СкладПоступления.АвтоОтметкаНезаполненного 	= Истина;
		Элементы.Перевозка_ДатаПоступления.АвтоОтметкаНезаполненного 					= Истина;
	КонецЕсли;
...
Функция ЗапросПеревозка_Выгрузить(МассивСделки)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ЗаданиеНаПеревозкуРаспоряжения.Вес КАК Перевозка_ОбъемОтгрузки,
...
Процедура Перевозка_ПриИзменении_БазисаПоставки()
	...
	Если Перевозка_БазисПоставки = "CPT" Тогда 
		...
		Элементы.Перевозка_ОбъемПоступления.АвтоОтметкаНезаполненного = Истина;
		
	ИначеЕсли Перевозка_БазисПоставки = "EXW" Тогда 	
		Элементы.ГруппаПеревозкаEXW.Видимость = Истина;	
		Элементы.Перевозка_ОбъемПоступления.АвтоОтметкаНезаполненного = Ложь;
...
Функция СохранитьСправочник_Договор(Параметры)
	...
	ОбъектЗаписи.Статус 										= Перечисления.СтатусыДоговоровКонтрагентов.НеСогласован;
