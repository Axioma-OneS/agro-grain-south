...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СвойстваВерсии.Версия < 5 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Автоматическая оплата сервиса не поддерживается.'"), , , ,
			Отказ);
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_ОткрытьОписание(Команда) Экспорт
	...
	ПараметрыФормы.Вставить("КодТарифаПровайдера", Подписка.КодТарифаПровайдера);
	ПараметрыФормы.Вставить("КодТарифаОбслуживающейОрганизации", Подписка.КодТарифаОбслуживающейОрганизации);
...
Процедура ПоказатьПодписки(СоздаватьВариантыПродления)
	...
	Продления = ВсеПодпискиАбонента.НайтиСтроки(Новый Структура("ТипПодписки",
		Перечисления.ТипыПодписокСервиса.Продлевающая));
	...
	ОсновныеПодписки = ВсеПодпискиАбонента.НайтиСтроки(Новый Структура("ТипПодписки",
		Перечисления.ТипыПодписокСервиса.Основная));
	...
	Для Каждого Подписка Из ОсновныеПодписки Цикл
		...
		КонецЕсли;
		Подписка.Активная = (Подписка.ДатаПодключения <= ТекущаяДатаСеанса() И Подписка.ДатаОтключения
			>= ТекущаяДатаСеанса());
	...
	Для Каждого Строка Из ОсновныеПодпискиБезТарифовРасширений Цикл
		...
		Тариф = ТарифПодписки(Строка.КодТарифа, Строка.КодТарифаОбслуживающейОрганизации, ТарифыПодписок, Истина);
		...
		Если Результат.Количество() = 0 Тогда
			...
			ЭлементГруппы.КодТарифаОбслуживающейОрганизации = Строка.КодТарифаОбслуживающейОрганизации;
			ЭлементГруппы.КодТарифаПровайдера = Строка.КодТарифа;
		...
		ЭлементСостава.КодТарифаОбслуживающейОрганизации = Строка.КодТарифаОбслуживающейОрганизации;
		ЭлементСостава.КодТарифаПровайдера = Строка.КодТарифа;
		...
		Для Каждого РасширениеПодписки Из ВсеПодпискиАбонента.НайтиСтроки(Отбор) Цикл
			Тариф = ТарифПодписки(РасширениеПодписки.КодТарифа, РасширениеПодписки.КодТарифаОбслуживающейОрганизации, 
				ТарифыПодписок, Истина);
			ЭлементСостава = ЭлементГруппы.СоставГруппы.Добавить();
			ЭлементСостава.КодТарифа = Тариф.Код;
			ЭлементСостава.КодТарифаОбслуживающейОрганизации = РасширениеПодписки.КодТарифаОбслуживающейОрганизации;
			ЭлементСостава.КодТарифаПровайдера = РасширениеПодписки.КодТарифа;
	...
	Для Каждого Группа Из ГруппыПодписок Цикл
		...
		ДатаОкончанияДействияТарифа = ТарифыПодписок.Получить(Группа.КодТарифаПровайдера).ДатаОкончанияДействия;
		...
		ОсновнойТариф = ТарифПодписки(Группа.КодТарифаПровайдера, Группа.КодТарифа, ТарифыПодписок, Ложь);
		...
		Для Каждого Период Из ОсновнойТариф.ПериодыДействия Цикл
			...
			Для Каждого СтрокаСостава Из Группа.СоставГруппы Цикл
				Тариф = ТарифПодписки(СтрокаСостава.КодТарифаПровайдера, СтрокаСостава.КодТарифа, ТарифыПодписок, Ложь);
	...
	Для Каждого Группа Из АктивныеПодписки Цикл
		СоздатьЭлементыПодписки(Группа, СоздаватьВариантыПродления, ЦветаСтиля.ТекущиеПодпискиЦвет,
			Элементы.ГруппаАктивныеПодписки);
	КонецЦикла;
...
&НаСервере
Функция ТарифПодписки(КодТарифаПровайдера, КодТарифаОО, ТарифыПодписок, ВозвращатьТарифПровайдераПриОтсутствииТарифаОО)

	Тариф = ТарифыПодписок.Получить(КодТарифаПровайдера);
	Если ИспользоватьТарифыОО Тогда
		Поиск = ТарифыОО.НайтиСтроки(Новый Структура("Код", КодТарифаОО));
		Если Поиск.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(ТарифОО) Тогда
				...
				Тариф = ТарифОО;
			ИначеЕсли Не ВозвращатьТарифПровайдераПриОтсутствииТарифаОО Тогда
				Тариф = Неопределено;
			КонецЕсли;
		ИначеЕсли Не ВозвращатьТарифПровайдераПриОтсутствииТарифаОО Тогда
			Тариф = Неопределено;
...
Процедура СоздатьЭлементыПодписки(Группа, СоздаватьВариантыПродления, ЦветФонаПодписки, РодительскаяГруппа)
	...
	ДекорацияТариф.Заголовок = ?(Группа.Количество = 1, НаименованиеТарифа, СтрШаблон("%1 
																					  |Х %2", НаименованиеТарифа,
		Группа.Количество));
	...
	ДекорацияСрокДействия.Заголовок = ?(Группа.ДатаПодключения > ТекущаяДатаСеанса(), СтрШаблон(НСтр("ru = 'С %1
																									 |до %2'"), Формат(
		Группа.ДатаПодключения, "ДФ='d MMMM yyyy ''г.'''"), Формат(Группа.ДатаОтключения, "ДФ='d MMMM yyyy ''г.'''")),
	...
	Для Каждого СтрокаРасширения Из Расширения Цикл
		...
		ГруппаРасширение = Элементы.Добавить(ИмяГруппаРасширение + РасширенияГруппы.Индекс(СтрокаРасширения), Тип(
			"ГруппаФормы"), ГруппаРасширения);
		...
		ДекорацияРасширение = Элементы.Добавить(ИмяТариф + РасширенияГруппы.Индекс(СтрокаРасширения), Тип(
			"ДекорацияФормы"), ГруппаРасширение);
		...
		ДекорацияРасширение.Заголовок = ?(СтрокаРасширения.Количество = 1, 
			НаименованиеТарифаРасширения, СтрШаблон("%1 
													|Х %2",
			НаименованиеТарифаРасширения, СтрокаРасширения.Количество));
	...
	ТекстПродления = ?(Группа.ПериодыПродления.Количество() > 0, НСтр("ru = 'Продлить:'"), НСтр(
		"ru = 'Эту подписку нельзя продлить.'"));
	ДекорацияПродлитьНа = Элементы.Добавить(ИмяПродлитьНа, Тип("ДекорацияФормы"), ГруппаОсновная);
	ДекорацияПродлитьНа.Вид = ВидДекорацииФормы.Надпись;
	ДекорацияПродлитьНа.Заголовок = ТекстПродления;
	ДекорацияПродлитьНа.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Центр;
	...
	Для НомерВарианта = 0 По Группа.ПериодыПродления.Количество() - 1 Цикл
		...
		КомандаПродления.Заголовок = СтрШаблон(НСтр("ru = 'На %1
													|за %2 %3'"), НРег(ВариантПродления.Наименование),
			ВариантПродления.Сумма, Тарификация.ПредставлениеВалютыОплаты());

		Если Цел(НомерВарианта / 6) > НомерБлока Тогда
			НомерБлока = НомерБлока + 1;
			БлокКнопки = Элементы.Добавить("Кнопки_" + ИдентификаторГруппы + "_" + НомерБлока, Тип("ГруппаФормы"),
				ГруппаКнопки);
		...
		КонецЕсли;
		КнопкаПродления = Элементы.Добавить(ИмяЭлемента, Тип("КнопкаФормы"), БлокКнопки);
...
Процедура ОткрытьФормуОбработкиДанных()
	...
	ОткрытьФорму(ИмяФормыОбработкиОтвета, ПараметрыФормы, ЭтотОбъект, , , , ,
...
Функция ЗапроситьСчетНаОплату(ИдентификаторГруппы, КодПериода)
	...
	Возврат ЗапроситьСчетНаПодписку(КодПериода, Истина, СоставПокупки);

КонецФункции

&НаСервере
Функция ЗапроситьСчетПоВыбранномуТарифу(КодТарифа, КодПериода)
	...
	Возврат ЗапроситьСчетНаПодписку(КодПериода, Ложь, СоставПокупки);
...
&НаСервере
Функция ЗапроситьСчетНаПодписку(КодПериода, Продление, СоставПокупки)
...
&НаСервере 
Функция ЗапроситьПроизвольныйСчет(Запрос)

	Данные = НовыйСчет();
	Данные.Вставить("purchase_structure", Запрос);
	Возврат ОтправитьИПодписаться(Данные);
...
Процедура ВсеТарифыЗакрытие(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		Если ЗначениеЗаполнено(Результат.Запрос) И ЗапроситьПроизвольныйСчет(Результат.Запрос) Тогда
			ОткрытьФормуОбработкиДанных();
		ИначеЕсли ЗначениеЗаполнено(Результат.КодТарифа) 
				И ЗапроситьСчетПоВыбранномуТарифу(Результат.КодТарифа, Результат.КодПериода) Тогда
			ОткрытьФормуОбработкиДанных();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииОбслуживающейОрганизации()
	...
	КартинкаЭлектроннаяПочта = Новый ФорматированнаяСтрока(БиблиотекаКартинок.КонтактнаяИнформацияЭлектроннаяПочта, , ,
		, ЭлектроннаяПочтаСсылка);
	СтрокаЭлектроннаяПочта = Новый ФорматированнаяСтрока(ТекущаяОО.Почта, , , , ЭлектроннаяПочтаСсылка);

	ЕстьСтраницаВыбораТарифа = (ПрограммныйИнтерфейсСервиса.СтраницаВыбораТарифаОбслуживающейОрганизации(КодОО, Ложь)
		<> Неопределено);
	Элементы.ГруппаДругиеТарифы.Видимость = ТекущаяОО.РазрешеноАвтоматическоеВыставлениеСчетов
		И ЕстьСтраницаВыбораТарифа;
