...
Процедура ПараметрыНациональныйКаталогПриИзменении()
	...
	Если НЕ (ЗначениеЗаполнено(apikey) И ЗначениеЗаполнено(party_id)) Тогда
...
Процедура ПараметрыДоступаНациональныйКаталогНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОткрытия    = Новый Структура("ПараметрыДоступаНациональныйКаталог", ПараметрыДоступаНациональныйКаталог());
...
Процедура ВыгрузитьНоменклатуру(Команда)
	...
	Если ВыгружатьВНациональныйКаталог И НЕ ((ЗначениеЗаполнено(apikey) И ЗначениеЗаполнено(party_id))) Тогда
	...
	Если ТипЗнч(Задания) = Тип("Массив") И Задания.Количество() Тогда
		...
		Если Индекс >= 0 Тогда
			...
			Если Задание.РегламентноеЗадание <> РегламентноеЗадание 
				ИЛИ Сообщение.Организация = XMLСтрока(Организация) Тогда
...
&НаКлиенте
Функция ОбновитьПараметрНастройкиНаКлиенте(ОбновляемыеПараметры)
	
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ОбновитьПараметрыНастройкиВыгрузки(УникальныйИдентификатор, Организация, ОбновляемыеПараметры, ВыгружатьВНациональныйКаталог));
	ПроверкаПроблемБыстро();
...
&НаСервереБезКонтекста
Функция ОбновитьПараметрыНастройкиВыгрузки(Знач УникальныйИдентификатор, Знач Организация, Знач ОбновляемыеПараметры, Знач ОбновитьСтатистику)
	...
	Если ОбновитьСтатистику Тогда
		Результат.Вставить("ПодсчетСтатистикиРезультатов", РаботаСНоменклатуройСлужебный.ПолучитьСтатистикуРезультатов(УникальныйИдентификатор, Организация));
	КонецЕсли;
