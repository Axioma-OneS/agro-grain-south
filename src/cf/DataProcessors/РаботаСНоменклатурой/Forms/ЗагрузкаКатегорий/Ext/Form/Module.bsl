...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ЭтоВыгрузкаВНациональныйКаталог",  ТолькоКатегорииНациональногоКаталога);
...
Процедура ОчиститьСтрокиПоискаКатегорий()
	...
	Если ТолькоКатегорииНациональногоКаталога Тогда
		Идентификатор = ИдентификаторыВыбранныхКатегорий.ВыгрузитьЗначения();
	КонецЕсли;
...
Процедура НастроитьДоступностьКнопкиВыбора(ТекущиеДанные)
	...
	Иначе
		Элементы.ВыбратьКатегорию.Доступность = Не (ТекущиеДанные.КоличествоПодчиненных > 0 
			И ОдиночныйРежимВыбора
			И РежимВыбораКатегорий 
			И НЕ РазрешеноВыбиратьГруппы);
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементовПриДлительнойОперации()
	...
	Элементы.ТолькоКатегорииНациональногоКаталога.Доступность = ДлительнаяОперацияНеАктивна;
			...
			Если КэшКатегорий.Свойство("_" + ИдентификаторКатегории) Тогда
				...
				Если ВеткаРодителя <> Неопределено И ТекущаяВетка <> Неопределено И ВеткаРодителя.ДочерниеПодгружены Тогда
					...
					Если ЭтоСозданиеФормы
						...
						И (Не ОдиночныйРежимВыбора 
						ИЛИ ТолькоКатегорииНациональногоКаталога) Тогда
...
Процедура ПостроитьДеревоКатегорийЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ДополнительныеПараметры.ЭтоСозданиеФормы
		...
		Если ТолькоКатегорииНациональногоКаталога И ТипЗнч(ИдентификаторКатегории) = Тип("Строка") Тогда
			ИдентификаторКатегории = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторКатегории);
		КонецЕсли;
	...
	Иначе
		...
		ПроставитьПризнакЗагрузки();
		Если ТолькоКатегорииНациональногоКаталога И ИдентификаторКатегории <> Неопределено Тогда
			Если ТипЗнч(ИдентификаторКатегории) = Тип("Строка") Тогда
				ИдентификаторКатегории = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторКатегории);
			КонецЕсли;
			ПоследняяКатегорияИзСписка = ИдентификаторКатегории[ИдентификаторКатегории.ВГраница()];
			ПроставитьПометкуВыбраннымКатегориям(Категории, ИдентификаторКатегории, ПоследняяКатегорияИзСписка);
...
Процедура ЗаполнитьДеревоКатегорийИзКэша(ЭтоПоискКатегорий, Знач ИдентификаторКатегории, СтруктураПоискаКатегорий,
	...
	ОткрытоОкноПриложения = Окно <> Неопределено;
